...
version: "0.6.7"
date: "Thu, 29 May 2025 10:30:52 +0100"
hash: "7e6ff6ae8c348cb6906d47090d9f60a14ee207b4"
---
...
\label{eq:statecomposition}
  \sigma &\equiv (\alpha, \recenthistory, \gamma, \delta, \eta, \iota, \kappa, \lambda, \rho, \tau, \varphi, \chi, \psi, \pi, \ready, \accumulated)\end{aligned}$$
...
We retain in state information on the most recent $\mathsf{H}$ blocks. This is used to preclude the possibility of duplicate or out of date work-reports from being submitted. $$\begin{aligned}
  ...
  \beefybelt &\in \seq{\H?} \\
  \lastaccout &\in \seq{\tup{\N_S, \H}}\end{aligned}$$

For each recent block, we retain its header hash, its state root, its accumulation-result [mmb]{.smallcaps} and the corresponding work-package hashes of each item reported (which is no more than the total number of cores, $\mathsf{C} = 341$).
...
We define the new Accumulation Output Log $\beefybelt$. This is formed from the block's accumulation-output sequence $\lastaccout'$ (defined in section [12](#sec:accumulation){reference-type="ref" reference="sec:accumulation"}), taking its root using the basic binary Merklization function ($\mathcal{M}_B$, defined in appendix [28](#sec:merklization){reference-type="ref" reference="sec:merklization"}) and appending it to the previous log value with the [mmb]{.smallcaps} append function ($\mathcal{A}$, defined in appendix [28.2](#sec:mmr){reference-type="ref" reference="sec:mmr"}). Throughout, the Keccak hash function is used to maximize compatibility with legacy systems: $$\begin{aligned}
  \using \mathbf{s} &= \sq{\se_4(s) \frown \se(h) \mid \tup{s, h} \orderedin \lastaccout'}\\
  \label{eq:buildbeefymap}
  \beefybelt' &\equiv \mathcal{A}(\beefybelt, \mathcal{M}_B(\mathbf{s}, \hash_K), \hash_K)\end{aligned}$$

The final state transition for $\recenthistory$ appends a new item including the new block's header hash, a Merkle commitment to the block's Accumulation Output Log and the set of work-reports made into it (for which we use the guarantees extrinsic, $\xtguarantees$). Formally: $$\begin{aligned}
...
The new state-trie root is the zero hash, $\H^0$, which is inaccurate but safe since $\recenthistory'$ is not utilized except to define the next block's $\recenthistory^\dagger$, which contains a corrected value for this, as per equation [\[eq:correctlaststateroot\]](#eq:correctlaststateroot){reference-type="ref" reference="eq:correctlaststateroot"}.
...
We require that the anchor block be within the last $\mathsf{H}$ blocks and that its details be correct by ensuring that it appears within our most recent blocks $\recenthistory^\dagger$: $$\begin{aligned}
  \forall x \in \mathbf{x} : \exists y \in \recenthistory^\dagger : x_a = y_h \wedge x_s = y_s \wedge x_b = \mathcal{M}_R(y_\mathbf{b})\!\!\!\!\!\!\end{aligned}$$
...
We require that the work-package of the report not be the work-package of some other report made in the past. We ensure that the work-package not appear anywhere within our pipeline. Formally: $$\begin{aligned}
  ...
  &\forall p \in \mathbf{p}, p \not\in \bigcup_{x \in \recenthistory}\keys{x_\mathbf{p}} \cup \bigcup_{x \in \accumulated}x \cup \mathbf{q} \cup \mathbf{a}\end{aligned}$$
...
We require that any segment roots mentioned in the segment-root lookup be verified as correct based on our recent work-package history and the present block: $$\begin{aligned}
  ...
  &\forall w \in \mathbf{w}: w_\mathbf{l} \subseteq \mathbf{p} \cup \bigcup_{b \in \recenthistory} b_\mathbf{p}\end{aligned}$$
...
We come to define the parallelized accumulation function $\Delta_*$ which, with the help of the single-service accumulation function $\Delta_1$, transforms an initial state-context, together with a sequence of work-reports and a dictionary of privileged always-accumulate services, into a tuple of the total gas utilized in [pvm]{.smallcaps} execution $u$, a posterior state-context $(\mathbf{d}', \mathbf{i}', \mathbf{q}', m', \mathbf{a}', v', \mathbf{x}')$ and the resultant accumulation-output pairings $\mathbf{b}$ and deferred-transfers $\wideparen{\mathbf{t}}$: $$\Delta_*\colon\left\{\;\begin{aligned}
...
Given the result of the top-level $\Delta_+$, we may define the posterior state $\chi'$, $\varphi'$ and $\iota'$ as well as the first intermediate state of the service-accounts $\accountspostacc$ and the Accumulation Output Log $\lastaccout'$: $$\begin{aligned}
...
The second intermediate state $\accountspostxfer$ may then be defined with all the deferred effects of the transfers applied followed by the last-accumulation record being updated for all accumulated services: $$\begin{aligned}
...
We limit the total size of the auditable *work-bundle*, containing the work-package, import and extrinsic items, together with all payloads, the authorizer configuration and the authorization token to around 13.6[mb]{.smallcaps}. This limit allows 2[mb]{.smallcaps}/s/core D$^{3}$L imports, and thus a full complement of 3,072 imports, assuming no extrinsics and a minimal work-package of 4[kb]{.smallcaps}: $$\begin{aligned}
  ...
  &\phantom{\mathsf{W}_B} = \mathsf{W}_M(\mathsf{W}_G + 1 + 32\ceil{\log_2(\mathsf{W}_T)}) + 4096 + 1\end{aligned}$$
...
Where: $$\begin{aligned}
  ...
  I(p, j) \equiv \,&\begin{cases}
    ...
    \multicolumn{2}{l}{\where (r, \mathbf{e}, u) = \Psi_R(
      j, p, \mathbf{o}, [S(w) \mid w \orderedin p_\wpNworkitems], \ell
    )}\\
...
We denote this set, as opcode indices rather than names, as $T$, which is a subset of all valid opcode indices $U$. We define the instruction opcode indices denoting the beginning of basic-blocks as $\varpi$: $$\varpi\equiv \left(\{0\} \cup \{n + 1 + \text{skip}(n) \mid n \in \N_{|\mathbf{c}|} \wedge \mathbf{k}_n = 1 \wedge \mathbf{c}_n \in T\}\right) \cap \{n \mid \mathbf{k}_n = 1 \wedge \mathbf{c}_n \in U\}$$
...
\end{aligned}$$
  ...
  ------------ -- --- ------------------------------------------------------------------------------
  30              1   ${{\memory}'}^{\circlearrowleft}_{\nu_X} = \nu_Y \bmod 2^8$
  (lr)1-4 31      1   ${{\memory}'}^{\circlearrowleft}_{\nu_X\dots+2} = \se_2(\nu_Y \bmod 2^{16})$
  (lr)1-4 32      1   ${{\memory}'}^{\circlearrowleft}_{\nu_X\dots+4} = \se_4(\nu_Y \bmod 2^{32})$
  (lr)1-4 33      1   ${{\memory}'}^{\circlearrowleft}_{\nu_X\dots+8} = \se_8(\nu_Y)$
  ------------ -- --- ------------------------------------------------------------------------------
...
\end{aligned}$$
  ...
  (lr)1-4 52      1   ${\registers}'_A = {\memory}^{\circlearrowleft}_{\nu_X}$
  (lr)1-4 53      1   ${\registers}'_A = \mathcal{X}_1({\memory}^{\circlearrowleft}_{\nu_X})$
  (lr)1-4 54      1   ${\registers}'_A = \de_2({\memory}^{\circlearrowleft}_{\nu_X\dots+2})$
  (lr)1-4 55      1   ${\registers}'_A = \mathcal{X}_2(\de_2({\memory}^{\circlearrowleft}_{\nu_X\dots+2}))$
  (lr)1-4 56      1   ${\registers}'_A = \de_4({\memory}^{\circlearrowleft}_{\nu_X\dots+4})$
  (lr)1-4 57      1   ${\registers}'_A = \mathcal{X}_4(\de_4({\memory}^{\circlearrowleft}_{\nu_X\dots+4}))$
  (lr)1-4 58      1   ${\registers}'_A = \de_8({\memory}^{\circlearrowleft}_{\nu_X\dots+8})$
  (lr)1-4 59      1   ${{\memory}'}^{\circlearrowleft}_{\nu_X} = {\registers}_A \bmod 2^8$
  (lr)1-4 60      1   ${{\memory}'}^{\circlearrowleft}_{\nu_X\dots+2} = \se_2({\registers}_A \bmod 2^{16})$
  (lr)1-4 61      1   ${{\memory}'}^{\circlearrowleft}_{\nu_X\dots+4} = \se_4({\registers}_A \bmod 2^{32})$
  (lr)1-4 62      1   ${{\memory}'}^{\circlearrowleft}_{\nu_X\dots+8} = \se_8({\registers}_A)$
  ------------ -- --- ---------------------------------------------------------------------------------------
...
\end{aligned}$$
  ...
  ------------ -- --- -------------------------------------------------------------------------------------------------
  70              1   ${{\memory}'}^{\circlearrowleft}_{{\registers}_A + \nu_X} = \nu_Y \bmod 2^8$
  (lr)1-4 71      1   ${{\memory}'}^{\circlearrowleft}_{{\registers}_A + \nu_X \dots+ 2} = \se_2(\nu_Y \bmod 2^{16})$
  (lr)1-4 72      1   ${{\memory}'}^{\circlearrowleft}_{{\registers}_A + \nu_X \dots+ 4} = \se_4(\nu_Y \bmod 2^{32})$
  (lr)1-4 73      1   ${{\memory}'}^{\circlearrowleft}_{{\registers}_A + \nu_X \dots+ 8} = \se_8(\nu_Y)$
  ------------ -- --- -------------------------------------------------------------------------------------------------
...
\end{aligned}$$
  ...
  (lr)1-4 101      1   $\begin{aligned}
                         ...
                         \end{aligned}$
  (lr)1-4 102      1   $\displaystyle{\registers}'_D = \sum_{i = 0}^{63}\mathcal{B}_{8}({\registers}_A)_i$
  (lr)1-4 103      1   $\displaystyle{\registers}'_D = \sum_{i = 0}^{31}\mathcal{B}_{4}({\registers}_A \bmod 2^{32})_i$
  (lr)1-4 104      1   $\displaystyle{\registers}'_D = \max(n \in \N_{65})\ \where \sum_{i = 0}^{i < n} \overleftarrow{\mathcal{B}}_{8}({\registers}_A)_i = 0$
  (lr)1-4 105      1   $\displaystyle{\registers}'_D = \max(n \in \N_{33})\ \where \sum_{i = 0}^{i < n} \overleftarrow{\mathcal{B}}_{4}({\registers}_A \bmod 2^{32})_i = 0$
  (lr)1-4 106      1   $\displaystyle{\registers}'_D = \max(n \in \N_{65})\ \where \sum_{i = 0}^{i < n} \mathcal{B}_{8}({\registers}_A)_{i} = 0$
  (lr)1-4 107      1   $\displaystyle{\registers}'_D = \max(n \in \N_{33})\ \where \sum_{i = 0}^{i < n} \mathcal{B}_{4}({\registers}_A \bmod 2^{32})_{i} = 0$
  (lr)1-4 108      1   ${\registers}'_D = \mathcal{Z}_{8}^{-1} (\mathcal{Z}_{1}({\registers}_A \bmod 2^8))$
  (lr)1-4 109      1   ${\registers}'_D = \mathcal{Z}_{8}^{-1} (\mathcal{Z}_{2}({\registers}_A \bmod 2^{16}))$
  (lr)1-4 110      1   ${\registers}'_D = {\registers}_A \bmod 2^{16}$
  (lr)1-4 111      1   $\forall i \in \N_8 : \se_8({\registers}'_D)_i = \se_8({\registers}_A)_{7-i}$
  ------------- -- --- ------------------------------------------------------------------------------------------------------------------------------------------------------
...
\end{aligned}$$
  ...
  ------------- -- --- ------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  120              1   ${{\memory}'}^{\circlearrowleft}_{{\registers}_B + \nu_X} = {\registers}_A \bmod 2^8$
  (lr)1-4 121      1   ${{\memory}'}^{\circlearrowleft}_{{\registers}_B + \nu_X \dots+ 2} = \se_2({\registers}_A \bmod 2^{16})$
  (lr)1-4 122      1   ${{\memory}'}^{\circlearrowleft}_{{\registers}_B + \nu_X \dots+ 4} = \se_4({\registers}_A \bmod 2^{32})$
  (lr)1-4 123      1   ${{\memory}'}^{\circlearrowleft}_{{\registers}_B + \nu_X \dots+ 8} = \se_8({\registers}_A)$
  (lr)1-4 124      1   ${\registers}'_A = {\memory}^{\circlearrowleft}_{{\registers}_B + \nu_X}$
  (lr)1-4 125      1   ${\registers}'_A = \mathcal{Z}_{8}^{-1} (\mathcal{Z}_{1}({\memory}^{\circlearrowleft}_{{\registers}_B + \nu_X}))$
  (lr)1-4 126      1   ${\registers}'_A = \de_2({\memory}^{\circlearrowleft}_{{\registers}_B + \nu_X\dots+2})$
  (lr)1-4 127      1   ${\registers}'_A = \mathcal{Z}_{8}^{-1} (\mathcal{Z}_{2}(\de_2({\memory}^{\circlearrowleft}_{{\registers}_B + \nu_X\dots+2})))$
  (lr)1-4 128      1   ${\registers}'_A = \de_4({\memory}^{\circlearrowleft}_{{\registers}_B + \nu_X\dots+4})$
  (lr)1-4 129      1   ${\registers}'_A = \mathcal{Z}_{8}^{-1} (\mathcal{Z}_{4}(\de_4({\memory}^{\circlearrowleft}_{{\registers}_B + \nu_X\dots+4})))$
  (lr)1-4 130      1   ${\registers}'_A = \de_8({\memory}^{\circlearrowleft}_{{\registers}_B + \nu_X\dots+8})$
  (lr)1-4 131      1   ${\registers}'_A = \mathcal{X}_4(({\registers}_B + \nu_X) \bmod 2^{32})$
  (lr)1-4 132      1   $\forall i \in \N_{64} : \mathcal{B}_{8}({\registers}'_A)_i = \mathcal{B}_{8}({\registers}_B)_i \wedge \mathcal{B}_{8}(\nu_X)_i$
  (lr)1-4 133      1   $\forall i \in \N_{64} : \mathcal{B}_{8}({\registers}'_A)_i = \mathcal{B}_{8}({\registers}_B)_i \oplus \mathcal{B}_{8}(\nu_X)_i$
  (lr)1-4 134      1   $\forall i \in \N_{64} : \mathcal{B}_{8}({\registers}'_A)_i = \mathcal{B}_{8}({\registers}_B)_i \vee \mathcal{B}_{8}(\nu_X)_i$
  (lr)1-4 135      1   ${\registers}'_A = \mathcal{X}_4(({\registers}_B \cdot \nu_X) \bmod 2^{32})$
  (lr)1-4 136      1   ${\registers}'_A = {\registers}_B < \nu_X$
  (lr)1-4 137      1   ${\registers}'_A = \mathcal{Z}_{8}({\registers}_B) < \mathcal{Z}_{8}(\nu_X)$
  (lr)1-4 138      1   ${\registers}'_A = \mathcal{X}_4(({\registers}_B \cdot 2^{\nu_X \bmod 32}) \bmod 2^{32})$
  (lr)1-4 139      1   ${\registers}'_A = \mathcal{X}_4(\floor{{\registers}_B \bmod 2^{32} \div 2^{\nu_X \bmod 32}})$
  (lr)1-4 140      1   ${\registers}'_A = \mathcal{Z}_{8}^{-1} (\floor{\mathcal{Z}_{4}({\registers}_B \bmod 2^{32} ) \div 2^{\nu_X \bmod 32}})$
  (lr)1-4 141      1   ${\registers}'_A = \mathcal{X}_4((\nu_X + 2^{32} - {\registers}_B) \bmod 2^{32})$
  (lr)1-4 142      1   ${\registers}'_A = {\registers}_B > \nu_X$
  (lr)1-4 143      1   ${\registers}'_A = \mathcal{Z}_{8}({\registers}_B) > \mathcal{Z}_{8}(\nu_X)$
  (lr)1-4 144      1   ${\registers}'_A = \mathcal{X}_4((\nu_X \cdot 2^{{\registers}_B \bmod 32}) \bmod 2^{32})$
  (lr)1-4 145      1   ${\registers}'_A = \mathcal{X}_4(\floor{\nu_X \bmod 2^{32} \div 2^{{\registers}_B \bmod 32}})$
  (lr)1-4 146      1   ${\registers}'_A = \mathcal{Z}_{8}^{-1} (\floor{\mathcal{Z}_{4}(\nu_X \bmod 2^{32}) \div 2^{{\registers}_B \bmod 32}})$
  ...
  (lr)1-4 151      1   ${\registers}'_A = \mathcal{X}_8(({\registers}_B \cdot 2^{\nu_X \bmod 64}) \bmod 2^{64})$
  (lr)1-4 152      1   ${\registers}'_A = \mathcal{X}_8(\floor{{\registers}_B \div 2^{\nu_X \bmod 64}})$
  (lr)1-4 153      1   ${\registers}'_A = \mathcal{Z}_{8}^{-1} (\floor{\mathcal{Z}_{8}({\registers}_B) \div 2^{\nu_X \bmod 64}})$
  ...
  (lr)1-4 157      1   ${\registers}'_A = \mathcal{Z}_{8}^{-1} (\floor{\mathcal{Z}_{8}(\nu_X) \div 2^{{\registers}_B \bmod 64}})$
  (lr)1-4 158      1   $\forall i \in \N_{64} : \mathcal{B}_{8}({\registers}'_A)_i = \mathcal{B}_{8}({\registers}_B)_{(i + \nu_X) \bmod 64}$
  (lr)1-4 159      1   $\forall i \in \N_{64} : \mathcal{B}_{8}({\registers}'_A)_i = \mathcal{B}_{8}(\nu_X)_{(i + {\registers}_B) \bmod 64}$
  (lr)1-4 160      1   ${\registers}'_A = \mathcal{X}_4(x) \ \where x \in \N_{2^{32}}, \forall i \in \N_{32} : \mathcal{B}_{4}(x)_i = \mathcal{B}_{4}({\registers}_B)_{(i + \nu_X) \bmod 32}$
  (lr)1-4 161      1   ${\registers}'_A = \mathcal{X}_4(x) \ \where x \in \N_{2^{32}}, \forall i \in \N_{32} : \mathcal{B}_{4}(x)_i = \mathcal{B}_{4}(\nu_X)_{(i + {\registers}_B) \bmod 32}$
  ------------- -- --- ------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
+:------------+:--+:--+:-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| 190         |   | 1 | ${\registers}'_D = \mathcal{X}_4(({\registers}_A + {\registers}_B) \bmod 2^{32})$                                                                                              |
+-------------+---+---+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 191 |   | 1 | ${\registers}'_D = \mathcal{X}_4(({\registers}_A + 2^{32} - ({\registers}_B \bmod 2^{32})) \bmod 2^{32})$                                                                      |
+-------------+---+---+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 192 |   | 1 | ${\registers}'_D = \mathcal{X}_4(({\registers}_A \cdot {\registers}_B) \bmod 2^{32})$                                                                                          |
+-------------+---+---+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
|             |   |   |   \end{cases}$                                                                                                                                                                 |
+-------------+---+---+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 197 |   | 1 | ${\registers}'_D = \mathcal{X}_4(({\registers}_A \cdot 2^{{\registers}_B \bmod 32}) \bmod 2^{32})$                                                                             |
+-------------+---+---+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 198 |   | 1 | ${\registers}'_D = \mathcal{X}_4(\floor{({\registers}_A \bmod 2^{32}) \div 2^{{\registers}_B \bmod 32}})$                                                                      |
+-------------+---+---+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 199 |   | 1 | ${\registers}'_D = \mathcal{Z}_{8}^{-1} (\floor{\mathcal{Z}_{4}({\registers}_A \bmod 2^{32}) \div 2^{{\registers}_B \bmod 32}})$                                               |
+-------------+---+---+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
+-------------+---+---+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 209 |   | 1 | ${\registers}'_D = \mathcal{Z}_{8}^{-1} (\floor{\mathcal{Z}_{8}({\registers}_A) \div 2^{{\registers}_B \bmod 64}})$                                                            |
+-------------+---+---+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4     |   | 1 | $\forall i \in \N_{64} : \mathcal{B}_{8}({\registers}'_D)_i = \mathcal{B}_{8}({\registers}_A)_i \wedge \mathcal{B}_{8}({\registers}_B)_i$                                      |
|             |   |   |                                                                                                                                                                                |
| 210         |   |   |                                                                                                                                                                                |
+-------------+---+---+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 211 |   | 1 | $\forall i \in \N_{64} : \mathcal{B}_{8}({\registers}'_D)_i = \mathcal{B}_{8}({\registers}_A)_i \oplus \mathcal{B}_{8}({\registers}_B)_i$                                      |
+-------------+---+---+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 212 |   | 1 | $\forall i \in \N_{64} : \mathcal{B}_{8}({\registers}'_D)_i = \mathcal{B}_{8}({\registers}_A)_i \vee \mathcal{B}_{8}({\registers}_B)_i$                                        |
+-------------+---+---+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 213 |   | 1 | ${\registers}'_D = \mathcal{Z}_{8}^{-1} (\floor{(\mathcal{Z}_{8}({\registers}_A) \cdot \mathcal{Z}_{8}({\registers}_B)) \div 2^{64}})$                                         |
+-------------+---+---+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 214 |   | 1 | ${\registers}'_D = \floor{({\registers}_A \cdot {\registers}_B) \div 2^{64}}$                                                                                                  |
+-------------+---+---+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 215 |   | 1 | ${\registers}'_D = \mathcal{Z}_{8}^{-1} (\floor{(\mathcal{Z}_{8}({\registers}_A) \cdot {\registers}_B) \div 2^{64}})$                                                          |
+-------------+---+---+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 216 |   | 1 | ${\registers}'_D = {\registers}_A < {\registers}_B$                                                                                                                            |
+-------------+---+---+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 217 |   | 1 | ${\registers}'_D = \mathcal{Z}_{8}({\registers}_A) < \mathcal{Z}_{8}({\registers}_B)$                                                                                          |
+-------------+---+---+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
|             |   |   |   \end{cases}$                                                                                                                                                                 |
+-------------+---+---+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 220 |   | 1 | $\forall i \in \N_{64} : \mathcal{B}_{8}({\registers}'_D)_{(i + {\registers}_B) \bmod 64} = \mathcal{B}_{8}({\registers}_A)_i$                                                 |
+-------------+---+---+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 221 |   | 1 | ${\registers}'_D = \mathcal{X}_4(x)\ \where x \in \N_{2^{32}}, \forall i \in \N_{32} : \mathcal{B}_{4}(x)_{(i + {\registers}_B) \bmod 32} = \mathcal{B}_{4}({\registers}_A)_i$ |
+-------------+---+---+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 222 |   | 1 | $\forall i \in \N_{64} : \mathcal{B}_{8}({\registers}'_D)_i = \mathcal{B}_{8}({\registers}_A)_{(i + {\registers}_B) \bmod 64}$                                                 |
+-------------+---+---+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 223 |   | 1 | ${\registers}'_D = \mathcal{X}_4(x)\ \where x \in \N_{2^{32}}, \forall i \in \N_{32} : \mathcal{B}_{4}(x)_i = \mathcal{B}_{4}({\registers}_A)_{(i + {\registers}_B) \bmod 32}$ |
+-------------+---+---+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 224 |   | 1 | $\forall i \in \N_{64} : \mathcal{B}_{8}({\registers}'_D)_i = \mathcal{B}_{8}({\registers}_A)_i \wedge \lnot \mathcal{B}_{8}({\registers}_B)_i$                                |
+-------------+---+---+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 225 |   | 1 | $\forall i \in \N_{64} : \mathcal{B}_{8}({\registers}'_D)_i = \mathcal{B}_{8}({\registers}_A)_i \vee \lnot \mathcal{B}_{8}({\registers}_B)_i$                                  |
+-------------+---+---+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 226 |   | 1 | $\forall i \in \N_{64} : \mathcal{B}_{8}({\registers}'_D)_i = \lnot ( \mathcal{B}_{8}({\registers}_A)_i \oplus \mathcal{B}_{8}({\registers}_B)_i )$                            |
+-------------+---+---+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 227 |   | 1 | ${\registers}'_D = \mathcal{Z}_{8}^{-1} (\max ( \mathcal{Z}_{8}({\registers}_A), \mathcal{Z}_{8}({\registers}_B) ))$                                                           |
+-------------+---+---+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 228 |   | 1 | ${\registers}'_D = \max ( {\registers}_A, {\registers}_B )$                                                                                                                    |
+-------------+---+---+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 229 |   | 1 | ${\registers}'_D = \mathcal{Z}_{8}^{-1} (\min ( \mathcal{Z}_{8}({\registers}_A), \mathcal{Z}_{8}({\registers}_B) ))$                                                           |
+-------------+---+---+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 230 |   | 1 | ${\registers}'_D = \min ( {\registers}_A, {\registers}_B )$                                                                                                                    |
+-------------+---+---+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
  ...
  **Gas usage**             
  ...
  $g = 10$                  $\begin{aligned}
                                ...
                                \using \mathbf{z} &= \begin{cases}
                                  ...
                                  \error &\otherwise
                                \end{cases} \\
                                (\varepsilon', \registers'_7, (\mathbf{x}'_\mathbf{u})_{(m, \mathbf{a}, v, \mathbf{z})}) &= \begin{cases}
                                  (\panic, \registers_7, (\mathbf{x}_\mathbf{u})_{(m, \mathbf{a}, v, \mathbf{z})}) &\when \{\mathbf{z}, \mathbf{a}\} \ni \error \\
                                  (\blacktriangleright, \mathtt{HUH}, (\mathbf{x}_\mathbf{u})_{(m, \mathbf{a}, v, \mathbf{z})}) &\otherwhen \mathbf{x}_s \ne (\mathbf{x}_\mathbf{u})_m\\
                                  (\blacktriangleright, \mathtt{WHO}, (\mathbf{x}_\mathbf{u})_{(m, \mathbf{a}, v, \mathbf{z})}) &\otherwhen (m, v) \not\in (\N_S, \N_S) \\
                                  (\blacktriangleright, \mathtt{OK}, \tuple{m, \mathbf{a}, v, \mathbf{z}}) &\otherwise \\
                                \end{cases}
                              \end{aligned}$
  (lr)1-1(lr)2-2            
  `assign` = 15             
  $g = 10$                  $\begin{aligned}
                                ...
                                \using \mathbf{q} &= \begin{cases}
                                  ...
                                  \error &\otherwise
                                \end{cases} \\
                                (\varepsilon', \registers'_7, (\mathbf{x}'_\mathbf{u})_\mathbf{q}[c], (\mathbf{x}'_\mathbf{u})_\mathbf{a}[c]) &= \begin{cases}
                                  (\panic, \registers_7, (\mathbf{x}_\mathbf{u})_\mathbf{q}[c], (\mathbf{x}_\mathbf{u})_\mathbf{a}[c]) &\when \mathbf{q} = \error \\
                                  (\blacktriangleright, \mathtt{HUH}, (\mathbf{x}_\mathbf{u})_\mathbf{q}[c], (\mathbf{x}_\mathbf{u})_\mathbf{a}[c]) &\otherwhen \mathbf{x}_s \ne (\mathbf{x}_\mathbf{u})_\mathbf{a}[c]\\
                                  (\blacktriangleright, \mathtt{CORE}, (\mathbf{x}_\mathbf{u})_\mathbf{q}[c], (\mathbf{x}_\mathbf{u})_\mathbf{a}[c]) &\otherwhen c \ge \mathsf{C} \\
                                  (\blacktriangleright, \mathtt{OK}, \mathbf{q}, a) &\otherwise \\
                                \end{cases} \\
                              \end{aligned}$
  (lr)1-1(lr)2-2            
  `designate` = 16          
  $g = 10$                  $\begin{aligned}
                                ...
                                (\varepsilon', \registers'_7, (\mathbf{x}'_\mathbf{u})_\mathbf{i}) &= \begin{cases}
                                  ...
                                  (\blacktriangleright, \mathtt{HUH}, (\mathbf{x}_\mathbf{u})_\mathbf{i}) &\otherwhen \mathbf{x}_s \ne (\mathbf{x}_\mathbf{u})_v\\
  ...
  $g = 10$                  $\begin{aligned}
                                ...
                                (\varepsilon', \registers'_7, \mathbf{x}'_i, (\mathbf{x}'_\mathbf{u})_\mathbf{d}) &\equiv \begin{cases}
                                  ...
                                  (\blacktriangleright, \mathtt{HUH}, \mathbf{x}_i, (\mathbf{x}_\mathbf{u})_\mathbf{d}) &\otherwhen f \ne 0 \wedge \mathbf{x}_s \ne (\mathbf{x}_\mathbf{u})_m \\
                                  (\blacktriangleright, \mathtt{CASH}, \mathbf{x}_i, (\mathbf{x}_\mathbf{u})_\mathbf{d}) &\otherwhen \mathbf{s}_b < (\mathbf{x}_\mathbf{s})_t \\
                                  (\blacktriangleright, \mathbf{x}_i, i^*, (\mathbf{x}_\mathbf{u})_\mathbf{d} \cup \mathbf{d}) &\otherwise \\
...
:   Log of recent activity. See equation [\[eq:recenthistory\]](#eq:recenthistory){reference-type="ref" reference="eq:recenthistory"}.
    ...
    :   The Merkle mountain belt for accumulating Accumulation outputs.
...
:   The privileged service indices.
    ...
    :   The indices of the services able to assign each core's authorizer queue.
...
:   The most recent Accumulation outputs. See equation [\[eq:lastaccout\]](#eq:lastaccout){reference-type="ref" reference="eq:lastaccout"}.
...
:   The total gas allocated across for all Accumulation. Should be no smaller than $\mathsf{G}_A\cdot\mathsf{C} + \sum_{g \in \mathcal{V}(\alwaysaccers)}(g)$.
