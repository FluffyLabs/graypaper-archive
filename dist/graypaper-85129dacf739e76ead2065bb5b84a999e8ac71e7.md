...
version: "0.6.3"
date: "Mon, 3 Mar 2025 16:44:08 +0800"
hash: "85129dacf739e76ead2065bb5b84a999e8ac71e7"
---
...
As mentioned earlier, the header's epoch marker $\mathbf{H}_e$ is either empty or, if the block is the first in a new epoch, then a tuple of the next and current epoch randomness, along with a sequence of Bandersnatch keys defining the Bandersnatch validator keys ($k_b$) beginning in the next epoch. Formally: $$\begin{aligned}
...
We finally come to define a *work result*, $\mathbb{L}$, which is the data conduit by which services' states may be altered through the computation done within a work-package.

$$\label{eq:workresult}
  \mathbb{L} \equiv (s \in \N_S, c \in \H, y \in \H, g \in \N_G, \wlNdata \in \Y \cup \mathbb{J})$$
...
Finally, there is the actual output datum or error of the execution of the code $\wlNdata$, which may be either an octet sequence in case it was successful, or a member of the set $\mathbb{J}$, if not. This latter set is defined as the set of possible errors, formally: $$\mathbb{J} \in \{ \oog, \panic, \badexports, \token{BAD}, \token{BIG} \}$$
...
Finally, we denote the set of service/hash pairs, utilized as a service-indexed commitment to the accumulation output, as $B$: $$B\equiv \{(\N_S, \H)\}$$
...
We define the outer accumulation function $\Delta_+$ which transforms a gas-limit, a sequence of work-reports, an initial partial-state and a dictionary of services enjoying free accumulation, into a tuple of the number of work-results accumulated, a posterior state-context, the resultant deferred-transfers and accumulation-output pairings: $$\Delta_+\colon\left\{\begin{aligned}
    &(\N_G, \seq{\mathbb{W}}, \partialstate, \dict{\N_S}{\N_G}) \to (\N, \partialstate, \defxfers, B) \\
...
We come to define the parallelized accumulation function $\Delta_*$ which, with the help of the single-service accumulation function $\Delta_1$, transforms an initial state-context, together with a sequence of work-reports and a dictionary of privileged always-accumulate services, into a tuple of the total gas utilized in [pvm]{.smallcaps} execution $u$, a posterior state-context $(\mathbf{x}', \mathbf{d}', \mathbf{i}', \mathbf{q}')$ and the resultant accumulation-output pairings $\mathbf{b}$ and deferred-transfers $\wideparen{\mathbf{t}}$: $$\Delta_*\colon\left\{\;\begin{aligned}
    &(\partialstate, \seq{\mathbb{W}}, \dict{\N_S}{\N_G}) \to (\N_G, \partialstate, \defxfers, B) \\
...
The second intermediate state $\accountspostxfer$ may then be defined with all the deferred effects of the transfers applied: $$\accountspostxfer = \{ s \mapsto \Psi_T(\accountspostacc, \tau', s, R(\mathbf{t}, s)) \mid (s \mapsto a) \in \accountspostacc \}$$
...
Given the resultant datum $d$ of some work-item, we define the item-to-result function $C$ as: $$C\colon\left\{\begin{aligned}
    ...
    (\tup{s, c, \mathbf{y}, a}, \wlNdata) &\mapsto \tup{s, c, \is{\wlNpayloadhash}{\mathcal{H}(\mathbf{y})}, \is{g}{a}, \wlNdata}
  \end{aligned}\right.$$
...
Formally: $$\label{eq:workresultfunction}
  \!\!\Xi \colon \left\{\begin{aligned}
    ...
    (p, c) &\mapsto \begin{cases}
        \error &\when \mathbf{o} \not\in \Y \\
        \tup{s, \is{x}{p_\wpNcontext}, c, \is{a}{p_\wpNauthorizer}, \mathbf{o}, \mathbf{l}, \mathbf{r}} \!\!\!\!\!&\otherwise
    \end{cases}
  \end{aligned}\right.\!\!\!\!\!\!\!$$
...
Where: $$\begin{aligned}
  ...
  I(p, j) \equiv \,&\begin{cases}
    ...
    \multicolumn{2}{l}{\where (r, \mathbf{e}) = \Psi_R\left(\,\begin{aligned}
      &w_c, w_g, w_s, h, w_\mathbf{y}, p_\wpNcontext,\\[2pt]
      &p_\wpNauthorizer, \mathbf{o}, S(w, \mathbf{l}), X(w), \ell
    \end{aligned}\,\right)
    }\\
...
The four instances where the [pvm]{.smallcaps} is utilized each expect to be able to pass argument data in and receive some return data back. We thus define the common [pvm]{.smallcaps} program-argument invocation function $\Psi_M$: $$\begin{aligned}
  \Psi_M&\colon \left\{\begin{aligned}
    ...
    (\mathbf{p}, \imath, \gascounter, \mathbf{a}, f, \mathbf{x}) &\mapsto \begin{cases}
      ...
      R(\Psi_H(\mathbf{c}, \imath, \gascounter, \registers, {\memory}, f, \mathbf{x})) &\when Y(\mathbf{p}) = (\mathbf{c}, \registers, {\memory})
    \end{cases}
  \end{aligned}\right.\\
  \where R&\colon (\varepsilon, \imath', \gascounter', \registers', {\memory}', \mathbf{x}') \mapsto \begin{cases}
    (\gascounter', \oog, \mathbf{x}') &\when \varepsilon = \oog \\
    (\gascounter', \memory'_{\registers'_{7}\dots+\registers'_{8}}, \mathbf{x}') &\when \varepsilon = \halt \wedge \mathbb{Z}_{\registers'_{7}\dots+\registers'_{8}} \subset \mathbb{V}_{{\memory}'} \\
...
$$\begin{aligned}
  ...
  \label{eq:isauthorizedmutator}F \in \Omega\ang{\{\}} &\colon
    ...
    \end{cases}\end{aligned}$$ Note for the Is-Authorized host-call dispatch function $F$ in equation [\[eq:isauthorizedmutator\]](#eq:isauthorizedmutator){reference-type="ref" reference="eq:isauthorizedmutator"}, we elide the host-call context since, being essentially stateless, it is always $\none$.
...
Unlike the other invocation functions, the Refine invocation function implicitly draws upon some recent service account state item $\delta$. The specific block from which this comes is not important, as long as it is no earlier than its work-package's lookup-anchor block. It explicitly accepts the work-package $p$ and the index of the work item to be refined, $i$. Additionally, the authorizer output $\mathbf{o}$ is provided together with all work items' import segments $\overline{\mathbf{i}}$ and an export segment offset $\varsigma$. It results in either some error $\mathbb{J}$ or a pair of the refinement output blob and the export sequence. Formally: $$\begin{aligned}
  &\Psi_R \colon \left\{\begin{aligned}
    ...
    (i, p, \mathbf{o}, \overline{\mathbf{i}}, \varsigma) &\mapsto \begin{cases}
      ...
      (\token{BIG}, []) &\otherwhen |\Lambda(\delta[w_s], (p_\mathbf{x})_t, w_\wiNcodehash)| > \mathsf{W}_C \\
      &\otherwise: \\
  ...
  **Gas usage**              
  ...
  $g = 10$                   $\begin{aligned}
                                 ...
                                 \using \mathbf{x} &= \begin{cases}
                                   \mathcal{P}_{\mathsf{W}_G}({\memory}^{\circlearrowleft}_{p\dots+z}) &\when \N_{p\dots+z} \subseteq \mathbb{V}_\memory\\
                                   \error &\otherwise
                                 \end{cases}\\
  ...
  $g = 10$                   $\begin{aligned}
                                 ...
                                 (\varepsilon', \registers'_7, {\memory}') &\equiv \begin{cases}
                                   ...
                                   \multicolumn{2}{l}{\where {\memory}' = {\memory}\exc {{\memory}'}^{\circlearrowleft}_{o\dots+z} = (\mathbf{m}[n]_\mathbf{u})_{s\dots+z}}
                                 \end{cases} \\
                               \end{aligned}$
  (lr)1-1(lr)2-2             
  `poke` = 22                
  $g = 10$                   $\begin{aligned}
                                 ...
                                 (\varepsilon', \registers'_7, \mathbf{m}') &\equiv \begin{cases}
                                   ...
                                   \multicolumn{2}{l}{\where \mathbf{m}' = \mathbf{m} \exc (\mathbf{m}'[n]_\mathbf{u})_{o\dots+z} = {\memory}^{\circlearrowleft}_{s\dots+z}}
                                 \end{cases} \\
                               \end{aligned}$
  (lr)1-1(lr)2-2             
  `zero` = 23                
...
A block $\mathbf{B}$ is serialized as a tuple of its elements in regular order, as implied in equations [\[eq:block\]](#eq:block){reference-type="ref" reference="eq:block"}, [\[eq:extrinsic\]](#eq:extrinsic){reference-type="ref" reference="eq:extrinsic"} and [\[eq:header\]](#eq:header){reference-type="ref" reference="eq:header"}. For the header, we define both the regular serialization and the unsigned serialization $\se_U$. Formally: $$\begin{aligned}
  ...
  \se(x \in \mathbb{L}) &\equiv \se_4(x_s)\concat\se(x_c, x_l)\concat\se_8(x_g)\concat\se(O(x_\wlNdata))\\
  \se(x \in \mathbb{W}) &\equiv \se(x_s, x_x, x_c, x_a, \var{x_\wrNauthoutput}, \var{x_\mathbf{l}}, \var{x_\wrNresults}) \\
