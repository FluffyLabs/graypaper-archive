...
version: "0.7.0"
date: "Wed, 25 Jun 2025 11:25:08 +0100"
hash: "38c4e62d2226dce17d6340de56929010390da2a3"
---
...
$\edsignaturebase{k}{m} \subset \blob[64]$ is the set of valid Ed25519 signatures, defined by [@rfc8032], made through knowledge of a secret key whose public key counterpart is $k \in \hash$ and whose message is $m$. To aid readability, we denote the set of valid public keys $\edkey$.
...
We use $\blskey \subset \blob[144]$ to denote the set of public keys for the [bls]{.smallcaps} signature scheme, described by [@jofc-2004-14130], on curve [bls]{.smallcaps}- defined by [@bls12-381]. We correspondingly use the notation $\blssignaturebase{k}{m}$ to denote the set of valid [bls]{.smallcaps} signatures for public key $k \in \blskey$ and message $m \in \blob$.
...
\label{eq:transitionfunctioncomposition}
  ...
  \activity' &\prec \tup{\xtguarantees, \xtpreimages, \xtassurances, \xttickets, \thetime, \activeset', \activity, \theheader, \accumulationstatistics, \deferredtransfersstatistics}\!\!\!\!\!\!\!\!\end{aligned}$$
...
The header must contain a valid seal and valid [vrf]{.smallcaps} output. These are two signatures both using the current slot's seal key; the message data of the former is the header's serialization omitting the seal component $\H_\Nsealsig$, whereas the latter is used as a bias-resistant entropy source and thus its message must already have been fixed: we use the entropy stemming from the [vrf]{.smallcaps} of the seal signature. Formally: $$\begin{aligned}
  \nonumber \using i = \cyclic{\safroleticket[\H_\Ntimeslot]}\colon \\
...
Every block, each core has three validators uniquely assigned to guarantee work-reports for it. This is borne out with $\Cvalcount = 1,023$ validators and $\Ccorecount = 341$ cores, since $\nicefrac{\Cvalcount}{\Ccorecount} = 3$. The core index assigned to each of the validators, as well as the validators' Ed25519 keys are denoted by $\guarantorassignments$: $$\guarantorassignments \in \tuple{\sequence[\Cvalcount]{\coreindex}, \sequence[\Cvalcount]{\edkey}}$$
...
\label{eq:guaranteesextrinsic}
  ...
  \xtguarantees &= \sqorderby{(g_\xgNworkreport)_\wrNcore}{g \in \xtguarantees}\end{aligned}$$
...
No reports may be placed on cores with a report pending availability on it. A report is valid only if the authorizer hash is present in the authorizer pool of the core on which the work is reported. Formally: $$\label{eq:reportcoresareunusedortimedout}
  \forall \wrX \in \incomingreports :
    \reportspostguarantees\subb{\wrX_\wrNcore} = \none \wedge \wrX_\wrNauthorizer \in \authpool_{\wrX_\wrNcore}$$
...
The newly available work-reports, $\justbecameavailable$, are partitioned into two sequences based on the condition of having zero prerequisite work-reports. Those meeting the condition, $\justbecameavailable^!$, are accumulated immediately. Those not, $\justbecameavailable^Q$, are for queued execution. Formally: $$\begin{aligned}
  \justbecameavailable^! &\equiv \sq{\build{w}{w \orderedin \justbecameavailable, \len{(w_\wrNcontext)_\wcNprerequisites} = 0 \wedge w_\wrNsrlookup = \emset}} \\
  \justbecameavailable^Q &\equiv E(\sq{
    D(w) \mid
    w \orderedin \justbecameavailable,
    \len{(w_\wrNcontext)_\wcNprerequisites} > 0 \vee w_\wrNsrlookup \ne \emset
  }, \accumulatedcup)\!\!\!\!\\
  D(w) &\equiv (w, \set{(w_\wrNcontext)_\wcNprerequisites} \cup \keys{w_\wrNsrlookup})\end{aligned}$$
...
We define the queue-editing function $E$, which is essentially a mutator function for items such as those of $\ready$, parameterized by sets of now-accumulated work-package hashes (those in $\accumulated$). It is used to update queues of work-reports when some of them are accumulated. Functionally, it removes all entries whose work-report's hash is in the set provided as a parameter, and removes any dependencies which appear in said set. Formally: $$E\colon\abracegroup{
    ...
    &\tup{\mathbf{r}, \mathbf{x}} \mapsto \sq{\build{
      ...
      \begin{aligned}
        ...
        &(w_\wrNavspec)_\asNpackagehash \not\in \mathbf{x}
      \end{aligned}
    }}
  }$$

We further define the accumulation priority queue function $Q$, which provides the sequence of work-reports which are accumulatable given a set of not-yet-accumulated work-reports and their dependencies. $$Q\colon\abracegroup{
...
Finally, we define the mapping function $P$ which extracts the corresponding work-package hashes from a set of work-reports: $$P\colon\abracegroup{
    ...
    \mathbf{w} &\mapsto \set{
      \build{(w_\wrNavspec)_\asNpackagehash}{w \in \mathbf{w}}
    }
  }$$
...
Our formalisms begin by defining $\partialstate$ as a characterization of (i.e. values capable of representing) state components which are both needed and mutable by the accumulation process. This comprises the service accounts state (as in $\accountspre$), the upcoming validator keys $\stagingset$, the queue of authorizers $\authqueue$ and the privileges state $\privileges$. Formally: $$\label{eq:partialstate}
...
We denote the set characterizing a *deferred transfer* as $\defxfer$, noting that a transfer includes a memo component $\dxNmemo$ of $\Cmemosize = 128$ octets, together with the service index of the sender $\dxNsource$, the service index of the receiver $\dxNdest$, the balance to be transferred $\dxNamount$ and the gas limit $\dxNgas$ for the transfer. Formally: $$\begin{aligned}
  \label{eq:defxfer}
  \defxfer \equiv \tuple{
    \isa{\dxNsource}{\serviceid} ,
    \isa{\dxNdest}{\serviceid} ,
    \isa{\dxNamount}{\balance} ,
    \isa{\dxNmemo}{\blob[\Cmemosize]} ,
    \isa{\dxNgas}{\gas}
  }\end{aligned}$$

Finally, we denote the set of service/hash pairs, utilized as a service-indexed commitment to the accumulation output, as $B$: $$B\equiv \protoset{\tuple{\serviceid, \hash}} \qquad
  U\equiv \sequence{\tuple{\serviceid, \gas}}$$

We define the outer accumulation function $\accseq$ which transforms a gas-limit, a sequence of work-reports, an initial partial-state and a dictionary of services enjoying free accumulation, into a tuple of the number of work-results accumulated, a posterior state-context, the resultant deferred-transfers and accumulation-output pairings: $$\label{eq:accseq}
  \accseq\colon\abracegroup{
    &\tuple{\gas, \sequence{\workreport}, \partialstate, \dictionary{\serviceid}{\gas}} \to \tuple{\N, \partialstate, \defxfers, B, U} \\
    &\tup{g, \mathbf{w}, \psX, \mathbf{f}} \!\mapsto\! \begin{cases}
      ...
      \tup{i + j, \psX', \mathbf{t}^* \!\!\concat \mathbf{t}, \mathbf{b}^* \!\cup \mathbf{b}, \mathbf{u}^* \!\!\concat \mathbf{u}}\!\!\!\! &
        \text{o/w}\!\!\!\!\!\!\!\! \\
    \end{cases} \\
    &\quad\where i = \max(\Nmax{\len{\mathbf{w}} + 1}): \sum_{w \in \mathbf{w}\sub{\dots i}}\sum_{r \in w_\wrNdigests}(r_\wdNgaslimit) \le g \\
...
We come to define the parallelized accumulation function $\accpar$ which, with the help of the single-service accumulation function $\accone$, transforms an initial state-context, together with a sequence of work-reports and a dictionary of privileged always-accumulate services, into a tuple of the total gas utilized in [pvm]{.smallcaps} execution, a posterior state-context and the resultant accumulation-output pairings and deferred-transfers: $$\label{eq:accpar}
  \accpar\colon\abracegroup[\;]{\begin{aligned}
    &\tuple{\partialstate, \sequence{\workreport}, \dictionary{\serviceid}{\gas}} \to \tuple{\partialstate, \defxfers, B, U} \\
    &\tup{\psX, \mathbf{w}, \mathbf{f}} \mapsto \tup{
      \tup{
        \mathbf{d}', \mathbf{i}', \mathbf{q}', m', \mathbf{a}', v', \mathbf{z}'
      }, \concatall{\mathbf{t}}, \mathbf{b}, \mathbf{u}
    }\!\!\!\!\!\!\\
    &\text{where:}\\
    &\ \begin{aligned}
      \using \mathbf{s} &= \set{\build{
        \mathbf{r}_\wdNserviceindex
        }{
          w \in \mathbf{w}, \mathbf{r} \in w_\wrNdigests
        }} \cup \keys{\mathbf{f}} \\
      \mathbf{u} &= \sq{\build{
          \tup{s, \accone(\psX, \mathbf{w}, \mathbf{f}, s)_\aoNgasused}
        }{
      ...
      \mathbf{b} &= \set{\build{
          ...
          s \in \mathbf{s},\,
          b = \accone(\psX, \mathbf{w}, \mathbf{f}, s)_\aoNyield,\,
          b \ne \none
        }} \\
      \mathbf{t} &= \sq{\build{
          \accone(\psX, \mathbf{w}, \mathbf{f}, s)_\aoNdefxfers
        }{
          s \orderedin \mathbf{s}
        }} \\
      \mathbf{d}' &= P(
        (\psNaccounts \cup \mathbf{n}) \setminus \mathbf{m},
        \bigcup_{s \in \mathbf{s}} \accone(\psX, \mathbf{w}, \mathbf{f}, s)_\aoNprovisions
      ) \\
      &\tup{\psNaccounts, \psNstagingset, \psNauthqueue, \psNmanager, \psNalwaysaccers} = \psX \\
      \tup{m', \mathbf{a}^*, v^*, \mathbf{z}'} &=
        (\accone(\psX, \mathbf{w}, \mathbf{f}, m)_\aoNpoststate)_{
          \tup{\psNmanager, \psNassigners, \psNdelegator, \psNalwaysaccers}
        } \\
      \forall c \in \coreindex :
        \mathbf{a}'\sub{c} &= ((
          \accone(\psX, \mathbf{w}, \mathbf{f}, \mathbf{a}^*\sub{c})_\aoNpoststate
        )_\psNassigners)\sub{c} \\
      v' &= (
          \accone(\psX, \mathbf{w}, \mathbf{f}, v^*)_\aoNpoststate
        )_\psNdelegator \\
      \mathbf{i}' &= (
          \accone(\psX, \mathbf{w}, \mathbf{f}, \psNdelegator)_\aoNpoststate
      )_\psNstagingset \\
      \forall c \in \coreindex :
        \mathbf{q}'\sub{c} &= ((
          \accone(\psX, \mathbf{w}, \mathbf{f}, \psNassigners\sub{c})_\aoNpoststate
        )_\psNauthqueue)\sub{c} \\
      \mathbf{n} &= \bigcup_{s \in \mathbf{s}}( \set{
          (\accone(\psX, \mathbf{w}, \mathbf{f}, s)_\aoNpoststate)_\psNaccounts
            \setminus
      ...
      \mathbf{m} &= \bigcup_{s \in \mathbf{s}}(
        \keys{\psNaccounts}
          \setminus
        \keys{(\accone(\psX, \mathbf{w}, \mathbf{f}, s)_\aoNpoststate)_\psNaccounts}
      )
    \end{aligned}
  \end{aligned}}$$

And $P$ is the preimage integration function, which transforms a dictionary of service states and a set of service/hash pairs into a new dictionary of service states. Preimage provisions into services which no longer exist or whose relevant request is dropped are disregarded: $$P\colon\abracegroup{
...
The single-service accumulation function, $\accone$, transforms an initial state-context, sequence of work-reports and a service index into an alterations state-context, a sequence of *transfers*, a possible accumulation-output and the actual [pvm]{.smallcaps} gas used. This function wrangles the work-items of a particular service from a set of work-reports and invokes [pvm]{.smallcaps} execution with said data: $$\begin{aligned}
  ...
  &\accone \colon \abracegroup[\;]{
    ...
    &\text{where:} \\
    &\ \begin{aligned}
      g &= \subifnone{\mathbf{f}\sub{s}, 0} + \!\!\!\!\sum_{w \in \mathbf{w}, \mathbf{r} \in w_\wrNdigests , \mathbf{r}_\wdNserviceindex = s}\!\!\!\!(\mathbf{r}_\wrNauthgasused) \\
      \mathbf{i} &= \sq{\build{
        \tup{\begin{alignedat}{3}
          \is{\otNresult}{\mathbf{r}_\wdNresult},\,
          \is{\otNgaslimit}{\mathbf{r}_\wdNgaslimit},\,
          \is{\otNpayloadhash}{\mathbf{r}_\wdNpayloadhash},\,
          \is{&\otNauthtrace\;&}{w_\wrNauthtrace&},\\
          \is{\otNsegroot}{(w_\wrNavspec)_\asNsegroot},\,
          \is{\otNpackagehash}{(w_\wrNavspec)_\asNpackagehash},\,
          \is{&\otNauthorizer\;&}{w_\wrNauthorizer&}
        \end{alignedat}}
      }{
        \begin{alignedat}{2}
          w& \orderedin \mathbf{w},&\\
          \mathbf{r}& \orderedin w_\wrNdigests,&\ \mathbf{r}_\wdNserviceindex = s
        \end{alignedat}
      }}
    \end{aligned}
  }\!\!\!\!\end{aligned}$$

This introduces $\operandtuple$, the set of wrangled *operand tuples*, used as an operand to the [pvm]{.smallcaps} Accumulation function $\Psi_A$: work-items (together with associated data in their work-packages) are rephrased into a sequence of such operand tuples $\mathbf{i}$. It also draws upon $\wdNgaslimit$, the gas limit implied by the work-reports and gas-privileges.
...
Given the result of the top-level $\accseq$, we may define the posterior state $\privileges'$, $\authqueue'$ and $\stagingset'$ as well as the first intermediate state of the service-accounts $\accountspostacc$ and the Accumulation Output Log $\lastaccout'$: $$\begin{aligned}
  ...
  &\tup{
    n, \psX', \mathbf{t}, \lastaccout', \mathbf{u}
...
We compose $\accumulationstatistics$, our accumulation statistics, which is a mapping from the service indices which were accumulated to the amount of gas used throughout accumulation and the number of work-items accumulated. Formally: $$\begin{aligned}
  ...
  \where &N(s) \equiv \sq{\build{r}{
    ...
    r \orderedin w_\wrNdigests ,
    r_\wdNserviceindex = s
  }}\end{aligned}$$

We have denoted the sequence of implied transfers as $\mathbf{t}$, ordered internally according to the source service's execution. We define a selection function $X$, which maps a desired destination service index into the sequence of transfers from those implied which target said service. It is ordered primarily according to the source service index and secondarily their order within $\mathbf{t}$. Formally: $$X\colon \abracegroup[\;]{
...
The second intermediate state $\accountspostxfer$ may then be defined with all the deferred effects of the transfers applied followed by the last-accumulation record being updated for all accumulated services: $$\begin{aligned}
  \mathbf{x}&= \set{\build{
    \kv{s}{\Psi_T(\accountspostacc, \thetime', s, X(s))}
  }{
    \kv{s}{a} \in \accountspostacc
  }} \\
  \label{eq:accountspostxferdef}
  \accountspostxfer &\equiv \set{ \build{ \kv{s}{a'} }{ \kv{s}{\tup{a, u}} \in \mathbf{x}}} \\
  &\where a' = \begin{cases}
    a \exc a'_\saNlastacc = \thetime' &\when s \in \keys{\accumulationstatistics} \\
    a &\otherwise
  \end{cases}\end{aligned}$$

Furthermore we build the deferred transfers statistics value $\deferredtransfersstatistics$ as the number of transfers and the total gas used in transfer processing for each *destination* service index. Formally: $$\begin{aligned}
  \label{eq:deferredtransfers}
  &\deferredtransfersstatistics \in \dictionary{\serviceid}{\tuple{\N, \gas}} \\
  &\textstyle \deferredtransfersstatistics \equiv \set{ \build { \kv{d}{\tup{\len{X(d)}, u}} }{
    \begin{aligned}
      X(d) &\ne \sq{},\\
      \exists a: \mathbf{x}\subb{d} &= \tup{a, u}
    \end{aligned}
  }}\end{aligned}$$

Note that $\Psi_T$ is defined in appendix [25.5](#sec:ontransferinvocation){reference-type="ref" reference="sec:ontransferinvocation"} such that it results in $\accountspostacc\subb{d}$, i.e. no difference to the account's intermediate state, if $X(d) = \sq{}$, i.e. said account received no transfers.
...
Where: $$\begin{aligned}
  ...
  I(\wpX, j) \equiv \,&\begin{cases}
    ...
    \multicolumn{2}{l}{\where \tup{r, \mathbf{e}, u} = \Psi_R(
      j, \wpX, \mathbf{o}, \sq{\build{S(w)}{w \orderedin \wpX_\wpNworkitems}}, \ell
    )}\\
...
The above relies on two functions, $S$ and $X$ which, respectively, define the import segment data and the extrinsic data for some work-item argument $\wiX$. We also define $J$, which compiles justifications of segment data: $$\begin{aligned}
    ...
    J(\wiX \in \workitem) &\equiv \sq{\build{\var{\fnmerklejustsubpath{0}{\mathbf{b}, n}}}{\merklizecd{\mathbf{b}} = L(r), \tup{r, n} \orderedin \wiX_\wiNimportsegments}}
  \end{aligned}$$
...
Unlike the other invocation functions, the Refine invocation function implicitly draws upon some recent service account state item $\accounts$. The specific block from which this comes is not important, as long as it is no earlier than its work-package's lookup-anchor block. It explicitly accepts the work-package $p$ and the index of the work item to be refined, $i$. Additionally, the authorizer trace $\mathbf{r}$ is provided together with all work items' import segments $\overline{\mathbf{i}}$ and an export segment offset $\segoff$. It results in either some error $\workerror$ or a pair of the refinement output blob and the export sequence. Formally: $$\begin{aligned}
  &\Psi_R \colon \abracegroup{
    ...
    \tup{i, p, \mathbf{r}, \overline{\mathbf{i}}, \segoff} &\mapsto \begin{cases}
      ...
      &\otherwise: \\
      &\quad\using a = \encode{i, w_\wiNserviceindex, \var{w_\wiNpayload}, \blake{p}}\;,\ \encode{\var{\mathbf{z}}, \mathbf{c}} = \histlookup(\accounts\subb{w_\wiNserviceindex}, (p_\wpNcontext)_\wcNlookupanchortime, w_\wiNcodehash)\\
      &\quad\also \tup{u, \mathbf{o}, \tup{\mathbf{m}, \mathbf{e}}} = \Psi_M(\mathbf{c}, 0, w_\wiNrefgaslimit, a, F, \tup{\emptyset, \sq{}})\ \colon\\
...
We define $\Psi_A$, the Accumulation invocation function as: $$\begin{aligned}
  ...
  \Psi_A& \colon\abracegroup{
    ...
    &\to \acconeout
    \\
    \tup{\aoNpoststate, t, s, g, \mathbf{o}} &\mapsto \begin{cases}
      \tup{
        \aoNpoststate,
        \is{\aoNdefxfers}{\sq{}},
        \is{\aoNyield}{\none},
      ...
      C(\Psi_M(\mathbf{c}, 5, g, \encode{t, s, \len{\mathbf{o}}}, F, I(\aoNpoststate, s)^2))
        &\otherwise \\
      \where \mathbf{c} = \aoNpoststate_\psNaccounts\subb{s}_\saNcode&
    \end{cases} \\
  }\\
  I&\colon\abracegroup{
    ...
    &\qquad\where \imNnextfreeid = \text{check}((\decode[4]{\blake{\encode{\imNid, \entropyaccumulator', \H_\Ntimeslot}}} \bmod (2^{32}-2^9)) + 2^8) \\
  }\\
  ...
  \Omega_G(\gascounter, \registers, \memory, \imXY) &\when n = \mathtt{gas} \\
    ...
    \Omega_\Aries(\gascounter, \registers, \memory, \imXY, \imX_\imNid) &\when n = \mathtt{provide} \\
...
We define the On-Transfer service-account invocation function as $\Psi_T$; it is somewhat similar to the Accumulation Invocation except that the only state alteration it facilitates are basic alteration to the storage of the subject account. No further transfers may be made, no privileged operations are possible, no new accounts may be created nor other operations done on the subject account itself. The function is defined as: $$\begin{aligned}
  ...
  \Psi_T &: \begin{cases}
    ...
    \tup{\mathbf{d}, t, s, \mathbf{t}} &\mapsto \begin{cases}
      \tup{\mathbf{s}, 0}
        &\when \mathbf{s}_\saNcode = \none \vee
          \len{\mathbf{s}_\saNcode} > \Cmaxservicecodesize \vee
          \mathbf{t} = \sq{} \\
      ...
      \multicolumn{2}{l}{
        \begin{aligned}
          \quad\where \tup{u, \mathbf{r}, \mathbf{s}'} &= \Psi_M(
            \mathbf{s}_\saNcode,
            10,
            \sum_{r \in \mathbf{t}}{(r_\dxNgas)},
            \encode{t, s, \len{\mathbf{t}}},
            F,
            \mathbf{s}
          ) \\
          \quad\also \mathbf{s} &= \mathbf{d}\subb{s} \exc \mathbf{s}_\saNbalance = \mathbf{d}\subb{s}_\saNbalance + \sum_{r \in \mathbf{t}}{r_\dxNamount} \\
        \end{aligned}
      }
    \end{cases} \\
  \end{cases} \\
  \also F &\in \contextmutator{\serviceaccount}\colon \tup{n, \gascounter, \registers, \memory, \mathbf{s}} \equiv \begin{cases}
    ...
    \tup{\blacktriangleright, \gascounter - 10, \sq{\registers_0, \dots, \registers_6, \mathtt{WHAT}, \registers_8, \dots}, \memory, \mathbf{s}} &\otherwise
  \end{cases}\end{aligned}$$
  ...
  **Gas usage**             
  ...
  $g = 10$                  $\begin{aligned}
                                ...
                                \using \mathbf{z} &= \begin{cases}
                                  ...
                                  \error &\otherwise
                                \end{cases} \\
                                \tup{\execst', \registers'_7, (\imX'_\imNstate)_{\tup{\psNmanager, \psNassigners, \psNdelegator, \psNalwaysaccers}}} &= \begin{cases}
                                  \tup{\panic, \registers_7, (\imX_\imNstate)_{\tup{\psNmanager, \psNassigners, \psNdelegator, \psNalwaysaccers}}} &\when \set{\mathbf{z}, \mathbf{a}} \ni \error \\
                                  \tup{\blacktriangleright, \mathtt{HUH}, (\imX_\imNstate)_{\tup{\psNmanager, \psNassigners, \psNdelegator, \psNalwaysaccers}}} &\otherwhen \imX_\imNid \ne (\imX_\imNstate)_\psNmanager\\
                                  \tup{\blacktriangleright, \mathtt{WHO}, (\imX_\imNstate)_{\tup{\psNmanager, \psNassigners, \psNdelegator, \psNalwaysaccers}}} &\otherwhen \tup{m, v} \not\in \tuple{\serviceid, \serviceid} \\
                                  \tup{\blacktriangleright, \mathtt{OK}, \tuple{m, \mathbf{a}, v, \mathbf{z}}} &\otherwise \\
                                \end{cases}
                              \end{aligned}$
  (lr)1-1(lr)2-2            
  `assign` = 15             
  ...
  `new` = 18                
  $g = 10$                  $\begin{aligned}
                                \using \sq{o, l, \saNminaccgas, \saNminmemogas, \saNgratis} &= \registers\subrange{7}{5} \\
                                ...
                                \tup{\execst', \registers'_7, \imX'_\imNnextfreeid, (\imX'_\imNstate)_\psNaccounts} &\equiv \begin{cases}
                                  ...
                                  \multicolumn{2}{l}{\quad \where i^* = \text{check}(2^8 + (\imX_\imNnextfreeid - 2^8 + 42) \bmod (2^{32} - 2^9))}\\
...
$$\begin{aligned}
  ...
  \\
  O(o \in \workerror \cup \blob) &\equiv \begin{cases}
...
Hence: $$\finitefield_{16} \equiv \frac{\finitefield_2\subb{x}}{x^{16}} + x^5 + x^3 + x^2 + 1$$
...
:   The (prior) state of the service accounts. See equation [\[eq:serviceaccounts\]](#eq:serviceaccounts){reference-type="ref" reference="eq:serviceaccounts"}.
    ...
    :   The post-accumulation, pre-transfer intermediate state. See equation [\[eq:accountspostaccdef\]](#eq:accountspostaccdef){reference-type="ref" reference="eq:accountspostaccdef"}.

    $\accountspostxfer$

    :   The post-transfer, pre-preimage integration intermediate state. See equation [\[eq:accountspostxferdef\]](#eq:accountspostxferdef){reference-type="ref" reference="eq:accountspostxferdef"}.
...
:   The period in timeslots after which an unreferenced preimage may be expunged. See `eject` definition in section [25.8](#sec:accumulatefunctions){reference-type="ref" reference="sec:accumulatefunctions"}.
