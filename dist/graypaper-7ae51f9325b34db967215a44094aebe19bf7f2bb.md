...
version: "0.3.7"
date: "Wed, 11 Sep 2024 13:31:45 +0900"
hash: "7ae51f9325b34db967215a44094aebe19bf7f2bb"
---
...
We first denote the set of deferred transfers as $\mathbb{T}$, noting that a transfer includes a memo component $m$ of 64 octets, together with the service index of the sender $s$, the service index of the receiver $d$, the amount of tokens to be transferred $a$ and the gas limit $g$ for the transfer. Formally: $$\begin{aligned}
...
We must now define the single-step [pvm]{.smallcaps} state-transition function $\Psi_1$: $$\Psi_1\colon \left\{\begin{aligned}
    (\Y, \seq{\N_R}, \N_R, \N_G, \seq{\N_R}_{13}, \mathbb{M}) &\to (\{\panic, \halt, \blacktriangleright\} \cup \{\fault, \host\} \times \N_R, \Z_G, \seq{\N_R}_{13}, \mathbb{M})\\
    (\mathbf{c}, \mathbf{j}, \imath, \xi, \omega, {\mu}) &\mapsto (\varepsilon, \imath', \xi', \omega', {\mu}')
  \end{aligned}\right.$$
...
Formally we begin by defining four utility functions for splitting some large sequence into a number of equal-sized sub-sequences and for reconstituting such subsequences back into a single large sequence: $$\begin{aligned}
  ...
  \forall n \in \N, k \in \N :\ &\text{unzip}_n(\mathbf{d} \in \Y_{k\cdot n}) \in \seq{\Y_n}_k \equiv \sq{ [\mathbf{d}_{j.k + i} \mid j \in \N_n] \mid i \in \N_k} \\
