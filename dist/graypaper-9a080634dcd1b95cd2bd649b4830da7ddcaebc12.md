...
version: "0.6.6"
date: "Mon, 5 May 2025 15:00:56 +0100"
hash: "9a080634dcd1b95cd2bd649b4830da7ddcaebc12"
---
...
\label{eq:transitionfunctioncomposition}
  ...
  \pi' &\prec (\xtguarantees, \xtpreimages, \xtassurances, \xttickets, \tau, \kappa', \pi, \mathbf{H}, \mathbf{I}, \mathbf{X})\!\!\!\!\!\!\!\!\end{aligned}$$
...
We come to define the parallelized accumulation function $\Delta_*$ which, with the help of the single-service accumulation function $\Delta_1$, transforms an initial state-context, together with a sequence of work-reports and a dictionary of privileged always-accumulate services, into a tuple of the total gas utilized in [pvm]{.smallcaps} execution $u$, a posterior state-context $(\mathbf{x}', \mathbf{d}', \mathbf{i}', \mathbf{q}')$ and the resultant accumulation-output pairings $\mathbf{b}$ and deferred-transfers $\wideparen{\mathbf{t}}$: $$\Delta_*\colon\left\{\;\begin{aligned}
    ...
    &\ \begin{aligned}
      ...
      \mathbf{d}' &= P((\mathbf{d} \cup \mathbf{n}) \setminus \mathbf{m}, \bigcup_{s \in \mathbf{s}} \Delta_1(\mathbf{o}, \mathbf{w}, \mathbf{f}, s)_\mathbf{p}) \\
...
This introduces $\operandtuple$, the set of wrangled *operand tuples*, used as an operand to the [pvm]{.smallcaps} Accumulation function $\Psi_A$: work-items (together with associated data in their work-packages) are rephrased into a sequence of such operand tuples $\mathbf{i}$. It also draws upon $\wlNgaslimit$, the gas limit implied by the work-reports and gas-privileges.
...
We limit the total size of the implied import and extrinsic items, together with all payloads, the authorizer configuration and the authorization token to 12[mb]{.smallcaps} in order to allow for around 2[mb]{.smallcaps}/s/core data throughput: $$\begin{aligned}
  ...
  &\begin{aligned}
    &\forall p\in \mathbb{P}: \Big(|p_\wpNauthtoken| + |p_\wpNauthconfig| +
...
\end{aligned}$$
  ...
  (lr)1-4 104      0   $\displaystyle{\registers}'_D = \max(n \in \N_{65})\ \where \sum_{i = 0}^{i < n} \overleftarrow{\mathcal{B}}_{8}({\registers}_A)_i = 0$
  (lr)1-4 105      0   $\displaystyle{\registers}'_D = \max(n \in \N_{33})\ \where \sum_{i = 0}^{i < n} \overleftarrow{\mathcal{B}}_{4}({\registers}_A \bmod 2^{32})_i = 0$
  (lr)1-4 106      0   $\displaystyle{\registers}'_D = \max(n \in \N_{65})\ \where \sum_{i = 0}^{i < n} \mathcal{B}_{8}({\registers}_A)_{i} = 0$
  (lr)1-4 107      0   $\displaystyle{\registers}'_D = \max(n \in \N_{33})\ \where \sum_{i = 0}^{i < n} \mathcal{B}_{4}({\registers}_A \bmod 2^{32})_{i} = 0$
...
We thus define the standard program code format $\mathbf{p}$, which includes not only the instructions and jump table (previously represented by the term $\mathbf{c}$), but also information on the state of the [ram]{.smallcaps} at program start. Given program blob $\mathbf{p}$ and argument data $\mathbf{a}$, we can decode the program code $\mathbf{c}$, registers $\registers$, and [ram]{.smallcaps} ${\memory}$ by invoking the standard initialization function $Y(\mathbf{p}, \mathbf{a})$: $$Y\colon\left\{\begin{aligned}
  (\Y, \Y_{:\mathsf{Z}_I}) &\to (\Y, \seq{\N_R}_{13}, \mathbb{M})? \\
...
The four instances where the [pvm]{.smallcaps} is utilized each expect to be able to pass argument data in and receive some return data back. We thus define the common [pvm]{.smallcaps} program-argument invocation function $\Psi_M$: $$\Psi_M\colon \left\{\begin{aligned}
    ...
    (\mathbf{p}, \imath, \gascounter, \mathbf{a}, f, \mathbf{x}) &\mapsto \begin{cases}
      ...
      R(\gascounter, \Psi_H(\mathbf{c}, \imath, \gascounter, \registers, {\memory}, f, \mathbf{x})) &\when Y(\mathbf{p}, \mathbf{a}) = (\mathbf{c}, \registers, {\memory})\\
      \multicolumn{2}{l}{
...
The Is-Authorized invocation is the first and simplest of the four, being totally stateless. It provides only host-call functions for inspecting its environment and parameters. It accepts as arguments only the core on which it should be executed, $c$. Formally, it is defined as $\Psi_I$: $$\begin{aligned}
  \Psi_I &\colon \left\{\begin{aligned}
    ...
    (p, c) &\mapsto \begin{cases}
      \tup{\token{BAD}, 0} &\when |p_\mathbf{c}| = \none \\
      \tup{\token{BIG}, 0} &\otherwhen |p_\mathbf{c}| > \mathsf{W}_A \\
      \tup{\mathbf{r}, u} &\otherwise \\
      \multicolumn{2}{l}{\where (u, \mathbf{r}, \none) = \Psi_M(p_\mathbf{c}, 0, \mathsf{G}_I, \se_2(c), F, \none)}\\
    \end{cases}\\
  \end{aligned}\right. \\
  \label{eq:isauthorizedmutator}F \in \Omega\ang{\{\}} &\colon
    (n, \gascounter, \registers, \memory) \mapsto \begin{cases}
      ...
      \Omega_Y(\gascounter, \registers, \memory, p, p, \none, \none, \none, \none, \none, \none, \none) &\when n = \mathtt{fetch} \\
...
Unlike the other invocation functions, the Refine invocation function implicitly draws upon some recent service account state item $\delta$. The specific block from which this comes is not important, as long as it is no earlier than its work-package's lookup-anchor block. It explicitly accepts the work-package $p$ and the index of the work item to be refined, $i$. Additionally, the authorizer trace $\mathbf{o}$ is provided together with all work items' import segments $\overline{\mathbf{i}}$ and an export segment offset $\varsigma$. It results in either some error $\mathbb{J}$ or a pair of the refinement output blob and the export sequence. Formally: $$\begin{aligned}
  &\Psi_R \colon \left\{\begin{aligned}
    ...
    (i, p, \mathbf{r}, \overline{\mathbf{i}}, \varsigma) &\mapsto \begin{cases}
      ...
      &\otherwise: \\
      &\quad\using a = \se(i, w_s, \var{w_\mathbf{y}}, \hash(p))\;,\ \se(\var{\mathbf{z}}, \mathbf{c}) = \Lambda(\delta[w_s], (p_\wpNcontext)_t, w_\wiNcodehash)\\
  ...
  &F \in \Omega\ang{(\dict{\N}{\mathbf{M}}, \seq{\G})} \colon
    (n, \gascounter, \registers, \memory, (\mathbf{m}, \mathbf{e})) \mapsto \begin{cases}
      ...
      \multicolumn{2}{l}{\also \overline{\mathbf{x}} = \sq{\sq{\mathbf{x} \mid (\hash(\mathbf{x}), |\mathbf{x}|) \orderedin w_\mathbf{x}} \mid w \orderedin p_\wpNworkitems}}
    \end{cases}\end{aligned}$$
...
The general functions are all broadly of the form $(\gascounter' \in \Z_G, \registers' \in \seq{\N_R}_{13}, \memory' \in \mathbb{M}) = \Omega_\square(\gascounter \in \N_G, \registers \in \seq{\N_R}_{13}, \memory \in \mathbb{M})$. Functions which have a result component which is equivalent to the corresponding argument may have said components elided in the description. Functions may also depend upon particular additional parameters.

Unlike the Accumulate functions in appendix [25.8](#sec:accumulatefunctions){reference-type="ref" reference="sec:accumulatefunctions"}, these do not mutate an accumulation context. Some, such as $\mathtt{write}$ mutate a service account and both accept and return some $\mathbf{s} \in \mathbb{A}$. Others are more general functions, such as $\mathtt{fetch}$ and do not assume any context but have a parameter list suffixed with an ellipsis to denote that any additional parameters may be taken and are provided transparently into its result. This allows it to be easily utilized in multiple [pvm]{.smallcaps} invocations.
  ...
  **Gas usage**    
  ...
  $g = 10$         $\begin{aligned}
                       \using \mathbf{v} &= \begin{cases}
                         ...
                         \se(p_\wpNauthcodehash, \var{p_\wpNauthconfig}) &\when p \ne \none \wedge \registers_{10} = 8 \\
                         p_\wpNauthtoken &\when p \ne \none \wedge \registers_{10} = 9 \\
                         p_\wpNcontext &\when p \ne \none \wedge \registers_{10} = 10 \\
                         \se(\var{\sq{S(w) \mid w \orderedin p_\wpNworkitems}}) &\when p \ne \none \wedge \registers_{10} = 11 \\
                         S(p_\wpNworkitems[\registers_{11}]) &\when p \ne \none \wedge \registers_{10} = 12 \wedge \registers_{11} < |p_\wpNworkitems| \\
                         \multicolumn{2}{l}{\where S(w) \equiv \se(\se_4(w_\wiNservice), w_\wiNcodehash, \se_8(w_\wiNrefgaslimit, w_\wiNaccgaslimit), \se_2(w_\wiNexportcount, |w_\wiNimportsegments|, |w_\wiNextrinsics|), \se_4(|w_\wiNpayload|))} \\
                         p_\wpNworkitems[\registers_{11}]_\wiNpayload &\when p \ne \none \wedge \registers_{10} = 13 \wedge \registers_{11} < |p_\wpNworkitems| \\
...
## B.7 Refine Functions {#sec:refinefunctions}
...
## B.8 Accumulate Functions {#sec:accumulatefunctions}
...
A block $\mathbf{B}$ is serialized as a tuple of its elements in regular order, as implied in equations [\[eq:block\]](#eq:block){reference-type="ref" reference="eq:block"}, [\[eq:extrinsic\]](#eq:extrinsic){reference-type="ref" reference="eq:extrinsic"} and [\[eq:header\]](#eq:header){reference-type="ref" reference="eq:header"}. For the header, we define both the regular serialization and the unsigned serialization $\se_U$. Formally: $$\begin{aligned}
  ...
  \se(x \in \operandtuple) &\equiv \se(x_\otNpackagehash, x_\otNsegroot, x_\otNauthorizer,  x_\otNpayloadhash, x_\otNgaslimit, O(x_\otNresult), \var{x_\otNauthtrace}) \\
...
Formally we begin by defining four utility functions for splitting some large sequence into a number of equal-sized sub-sequences and for reconstituting such subsequences back into a single large sequence: $$\begin{aligned}
  \forall n \in \N, k \in \N :\ &\text{split}_n(\mathbf{d} \in \Y_{k:n}) \in \seq{\Y_n}_k \equiv \sq{\mathbf{d}_{0\dots+n}, \mathbf{d}_{n\dots+n}, \cdots, \mathbf{d}_{(k-1)n\dots+n}} \\
  \forall n \in \N, k \in \N :\ &\text{join}_n(\mathbf{c} \in \seq{\Y_n}_k) \in \Y_{k:n} \equiv \mathbf{c}_0 \concat \mathbf{c}_1 \concat \dots \\
  \forall n \in \N, k \in \N :\ &\text{unzip}_n(\mathbf{d} \in \Y_{k:n}) \in \seq{\Y_n}_k \equiv \sq{ [\mathbf{d}_{j\cdot k + i} \mid j \orderedin \N_n] \mid i \orderedin \N_k} \\
  \forall n \in \N, k \in \N :\ &\text{lace}_n(\mathbf{c} \in \seq{\Y_n}_k) \in \Y_{k:n} \equiv \mathbf{d} \ \where \forall i \in \N_k, j \in \N_n: \mathbf{d}_{j\cdot k + i} = (\mathbf{c}_i)_j\end{aligned}$$
