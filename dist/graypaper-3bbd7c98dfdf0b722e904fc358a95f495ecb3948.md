...
version: "0.8.0"
date: "Mon, 2 Mar 2026 06:44:47 +0000"
hash: "3bbd7c98dfdf0b722e904fc358a95f495ecb3948"
---
...
Offender signatures must be similarly valid and reference work-reports with judgments and may not report keys which are already in the punish-set: $$\begin{aligned}
  ...
  \forall \tup{\xfNreporthash, \xfNvalidity, \xfNoffenderindex, \xfNsignature} &\in \xtfaults : \bigwedge \abracegroup{
    ...
    &\xfNsignature \in \edsignature{\xfNoffenderindex}{\mathsf{X}\sub{v} \concat \xfNreporthash}\\
  }\\
...
Where: $$\begin{aligned}
  ...
  I(\wpX, j) \equiv \,&\begin{cases}
    ...
    \multicolumn{2}{l}{\where \tup{r, \mathbf{e}, u} = \Psi_R(
      c, j, \wpX, \wrNauthtrace, S^\#(\wpX_\wpNworkitems), \ell
    )}\\
...
The inner [pvm]{.smallcaps} invocation host-calls, meanwhile, depend on an integrated [pvm]{.smallcaps} type, which we shall denote $\pvmguest$. It holds a [pvm]{.smallcaps} program blob, instruction counter, [ram]{.smallcaps} and a flag denoting whether gas was already charged for the currently executing basic block: $$\label{eq:pvmguest}
  \pvmguest \equiv \tuple{\isa{\pgNcode}{\blob}, \isa{\pgNram}{\ram}, \isa{\pgNpc}{\pvmreg}, \isa{\pgNgaschargedflag}{\bool}}$$
  ...
  **Gas usage**     
  ...
  `grow_heap` = 1   $\begin{aligned}
                        ...
                        \tup{\execst', \gascounter', \registers'_7} &\equiv \begin{cases}
                          ...
                          \tup{\blacktriangleright, \gascounter - 100, h} &\when \registers_7 \leq h \lor \registers_7 > b \\
...
These assume some refine context pair $\tup{\mathbf{m}, \mathbf{e}} \in \tuple{\dictionary{\N}{\pvmguest}, \sequence{\segment}}$, which are both initially empty. Other than the gas-counter which is explicitly defined, elements of [pvm]{.smallcaps} state are each assumed to remain unchanged by the host-call unless explicitly specified. $$\begin{aligned}
  ...
  **Gas usage**             
  ...
  $g = 10$                  $\begin{aligned}
                                ...
                                \end{cases} \\
                                \using \tup{c, i', g', \pgNgaschargedflag', \mathbf{w}', \mathbf{u}'} &= \Psi(\mathbf{m}\subb{n}_\pgNcode, \mathbf{m}\subb{n}_\pgNpc, g, \mathbf{m}\subb{n}_{\pgNgaschargedflag}, \mathbf{w}, \mathbf{m}\subb{n}_\pgNram)\\
