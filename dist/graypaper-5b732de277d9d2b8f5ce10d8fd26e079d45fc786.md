...
version: "0.5.2"
date: "Fri, 6 Dec 2024 17:51:26 +0100"
hash: "5b732de277d9d2b8f5ce10d8fd26e079d45fc786"
---
...
Finally, there is the output or error of the execution of the code $\mathbf{o}$, which may be either an octet sequence in case it was successful, or a member of the set $\mathbb{J}$, if not. This latter set is defined as the set of possible errors, formally: $$\mathbb{J} \in \{ \oog, \panic, \token{BAD}, \token{BIG} \}$$
...
A bit may only be set if the corresponding core has a report pending availability on it which has not timed out: $$\begin{aligned}
  &\forall a \in \xtassurances, c \in \N_\mathsf{C} :\\
  &\quad a_f[c] \Rightarrow \rho^\dagger[c] \ne \none \wedge \mathbf{H}_t \le \rho^\dagger[c]_t + \mathsf{U}\end{aligned}$$
...
We require that the anchor block be within the last $\mathsf{H}$ blocks and that its details be correct by ensuring that it appears within our most recent blocks $\beta$: $$\begin{aligned}
  \forall x \in \mathbf{x} : \exists y \in \beta : x_a = y_h \wedge x_s = y_s \wedge x_b = \mathcal{M}_R(y_\mathbf{b})\!\!\!\!\!\!\end{aligned}$$
...
Where: $$\begin{aligned}
  ...
  &\also h = \mathcal{H}(p)\,,\; w = p_\wpNworkitems[j]\,,\; \ell = \sum_{k < j}p_\wpNworkitems[k]_\wiNexportsegments\end{aligned}$$
...
Other than the gas-counter which is explicitly defined, elements of [pvm]{.smallcaps} state are each assumed to remain unchanged by the host-call unless explicitly specified. $$\begin{aligned}
  ...
  **Gas usage**                                       
  ...
  (lr)1-1(lr)2-2                                      
  `transfer` = 11                                     
  $g = 10 + \registers_8 + 2^{32}\cdot\registers_9$   $\begin{aligned}
