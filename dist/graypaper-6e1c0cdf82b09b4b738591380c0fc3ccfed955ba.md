...
hash: "6e1c0cdf82b09b4b738591380c0fc3ccfed955ba"
---
...
Finally, there is the output or error of the execution of the code $\mathbf{o}$, which may be either an octet sequence in case it was successful, or a member of the set $\mathbb{J}$, if not. This latter set is defined as the set of possible errors, formally: $$\mathbb{J} \in \{ \oog, \panic, \badexports, \token{BAD}, \token{BIG} \}$$
...
We require that the gas allotted for accumulation of each work item in each work-report respects its service's minimum gas requirements. We also require that all work-reports total allotted accumulation gas is no greater than the overall gas limit $\mathsf{G}_A$: $$\forall w \in \mathbf{w}:
...
We define the queue-editing function $E$, which is essentially a mutator function for items such as those of $\ready$, parameterized by sets of now-accumulated work-package hashes (those in $\accumulated$). It is used to update queues of work-reports when some of them are accumulated. Functionally, it removes all entries whose work-report's hash is in the parameter's keys, removes any dependencies which appear in said set and, crucially, removes any entries whose segment-root correspondences diverge from those in the dictionary. Formally: $$E\colon\left\{\begin{aligned}
...
The single-service accumulation function, $\Delta_1$, transforms an initial state-context, sequence of work-reports and a service index into an alterations state-context, a sequence of *transfers*, a possible accumulation-output and the actual [pvm]{.smallcaps} gas used. This function wrangles the work-items of a particular service from a set of work-reports and invokes [pvm]{.smallcaps} execution with said data: $$\begin{aligned}
  &\mathbb{O} \equiv \tuple{\isa{\mathbf{o}}{\Y \cup \mathbb{J}}, \isa{l}{\H}, \isa{k}{\H}, \isa{\mathbf{a}}{\Y}}\end{aligned}$$ $$\begin{aligned}
...
We limit both the total number of exported items and the total number of imported items to $\mathsf{W}_M = 2^{11}$: $$\forall p\in \mathbb{P}:
  \sum_{w\in p_\wpNworkitems} w_\wiNexportsegments \le \mathsf{W}_M \ \wedge\ 
  \sum_{w\in p_\wpNworkitems} |w_\wiNimportsegments| \le \mathsf{W}_M$$
...
The above relies on two functions, $S$ and $X$ which, respectively, define the import segment data and the extrinsic data for some work-item argument $w$. We also define $J$, which compiles justifications of segment data: $$\begin{aligned}
    ...
    J(w\in \mathbb{I}) &\equiv [\var{\mathcal{J}(\mathbf{s}, n)} \mid \mathcal{M}(\mathbf{s}) = L(r), (r, n) \orderedin w_\wiNimportsegments]
  \end{aligned}$$
...
We declare the general [pvm]{.smallcaps} function $\Psi$. We assume a single-step invocation function define $\Psi_1$ and define the full [pvm]{.smallcaps} recursively as a sequence of such mutations up until the single-step mutation results in a halting condition. $$\Psi\colon \left\{\begin{aligned}
    ...
    \end{cases} \\
    \where (\varepsilon, \imath', \gascounter', \registers', {\memory}') &= \Psi_1(\mathbf{c}, \mathbf{k}, \mathbf{j}, \imath, \gascounter, \registers, {\memory})\\
...
If the latter condition cannot be satisfied, then $(\panic, \imath, \gascounter, \registers, {\memory})$ is the result.
...
Where [ram]{.smallcaps} must be inspected and yet access is not possible, then machine state is unchanged, and the exit reason is a fault with the lowest address to be read which is inaccessible. More formally, let $\mathbf{a}$ be the set of indices, modulo $2^{32}$, in to which ${\memory}$ must be subscripted in order to calculate the result of $\Psi_1$. If $\mathbf{a} \not\subset \mathbb{V}_{\memory}$ then let $\varepsilon=\fault \times \text{min}(\mathbf{a} \setminus \mathbb{V}_{\memory})$.

Similarly, where [ram]{.smallcaps} must be mutated and yet mutable access is not possible, then machine state is unchanged, and the exit reason is a fault with the lowest address to be read which is inaccessible. More formally, let $\mathbf{a}$ be the set of indices in to which ${\memory}'$ must be subscripted in order to calculate the result of $\Psi_1$. If $\mathbf{a} \not\subset \mathbb{V}^*_{\memory}$ then let $\varepsilon=\fault \times \text{min}(\mathbf{a} \setminus \mathbb{V}^*_{\memory})$.
...
Note, the term $h$ above refers to the beginning of the heap, the second major section of memory as defined in equation [\[eq:memlayout\]](#eq:memlayout){reference-type="ref" reference="eq:memlayout"} as $2\mathsf{Z}_Z + Q(|\mathbf{o}|)$. If $\token{sbrk}$ instruction is invoked on a [pvm]{.smallcaps} instance which does not have such a memory layout, then $h = 0$.
...
\end{aligned}$$
  ...
  ------------- -- --- ----------------------------------------------------------------------------------------------------------------------------------
  110              0   ${{\memory}'}^{\circlearrowleft}_{{\registers}_B + \nu_X} = {\registers}_A \bmod 2^8$
  (lr)1-4 111      0   ${{\memory}'}^{\circlearrowleft}_{{\registers}_B + \nu_X \dots+ 2} = \se_2({\registers}_A \bmod 2^{16})$
  (lr)1-4 112      0   ${{\memory}'}^{\circlearrowleft}_{{\registers}_B + \nu_X \dots+ 4} = \se_4({\registers}_A \bmod 2^{32})$
  (lr)1-4 113      0   ${{\memory}'}^{\circlearrowleft}_{{\registers}_B + \nu_X \dots+ 8} = \se_8({\registers}_A)$
  (lr)1-4 114      0   ${\registers}'_A = {\memory}^{\circlearrowleft}_{{\registers}_B + \nu_X}$
  (lr)1-4 115      0   ${\registers}'_A = \mathcal{Z}_{8}^{-1} (\mathcal{Z}_{1}({\memory}^{\circlearrowleft}_{{\registers}_B + \nu_X}))$
  (lr)1-4 116      0   ${\registers}'_A = \de_2({\memory}^{\circlearrowleft}_{{\registers}_B + \nu_X\dots+2})$
  (lr)1-4 117      0   ${\registers}'_A = \mathcal{Z}_{8}^{-1} (\mathcal{Z}_{2}(\de_2({\memory}^{\circlearrowleft}_{{\registers}_B + \nu_X\dots+2})))$
  (lr)1-4 118      0   ${\registers}'_A = \de_4({\memory}^{\circlearrowleft}_{{\registers}_B + \nu_X\dots+4})$
  (lr)1-4 119      0   ${\registers}'_A = \mathcal{Z}_{8}^{-1} (\mathcal{Z}_{4}(\de_4({\memory}^{\circlearrowleft}_{{\registers}_B + \nu_X\dots+4})))$
  (lr)1-4 120      0   ${\registers}'_A = \de_8({\memory}^{\circlearrowleft}_{{\registers}_B + \nu_X\dots+8})$
  (lr)1-4 121      0   ${\registers}'_A = \mathcal{X}_4(({\registers}_B + \nu_X) \bmod 2^{32})$
  (lr)1-4 122      0   $\forall i \in \N_{64} : \mathcal{B}_{8}({\registers}'_A)_i = \mathcal{B}_{8}({\registers}_B)_i \wedge \mathcal{B}_{8}(\nu_X)_i$
  (lr)1-4 123      0   $\forall i \in \N_{64} : \mathcal{B}_{8}({\registers}'_A)_i = \mathcal{B}_{8}({\registers}_B)_i \oplus \mathcal{B}_{8}(\nu_X)_i$
  (lr)1-4 124      0   $\forall i \in \N_{64} : \mathcal{B}_{8}({\registers}'_A)_i = \mathcal{B}_{8}({\registers}_B)_i \vee \mathcal{B}_{8}(\nu_X)_i$
  (lr)1-4 125      0   ${\registers}'_A = \mathcal{X}_4(({\registers}_B \cdot \nu_X) \bmod 2^{32})$
  (lr)1-4 126      0   ${\registers}'_A = {\registers}_B < \nu_X$
  (lr)1-4 127      0   ${\registers}'_A = \mathcal{Z}_{8}({\registers}_B) < \mathcal{Z}_{8}(\nu_X)$
  (lr)1-4 128      0   ${\registers}'_A = \mathcal{X}_4(({\registers}_B \cdot 2^{\nu_X \bmod 32}) \bmod 2^{32})$
  (lr)1-4 129      0   ${\registers}'_A = \mathcal{X}_4(\floor{{\registers}_B \bmod 2^{32} \div 2^{\nu_X \bmod 32}})$
  (lr)1-4 130      0   ${\registers}'_A = \mathcal{Z}_{8}^{-1} (\floor{\mathcal{Z}_{4}({\registers}_B \bmod 2^{32} ) \div 2^{\nu_X \bmod 32}})$
  (lr)1-4 131      0   ${\registers}'_A = \mathcal{X}_4((\nu_X + 2^{32} - {\registers}_B) \bmod 2^{32})$
  (lr)1-4 132      0   ${\registers}'_A = {\registers}_B > \nu_X$
  (lr)1-4 133      0   ${\registers}'_A = \mathcal{Z}_{8}({\registers}_B) > \mathcal{Z}_{8}(\nu_X)$
  (lr)1-4 134      0   ${\registers}'_A = \mathcal{X}_4((\nu_X \cdot 2^{{\registers}_B \bmod 32}) \bmod 2^{32})$
  (lr)1-4 135      0   ${\registers}'_A = \mathcal{X}_4(\floor{\nu_X \div 2^{{\registers}_B \bmod 32}})$
  (lr)1-4 136      0   ${\registers}'_A = \mathcal{Z}_{8}^{-1} (\floor{\mathcal{Z}_{4}(\nu_X) \div 2^{{\registers}_B \bmod 32}})$
  (lr)1-4 137      0   ${\registers}'_A = \begin{cases}
                           \nu_X &\when {\registers}_B = 0\\
                           {\registers}_A &\otherwise
                         \end{cases}$
  (lr)1-4 138      0   ${\registers}'_A = \begin{cases}
                           \nu_X &\when {\registers}_B \ne 0\\
                           {\registers}_A &\otherwise
                         \end{cases}$
  (lr)1-4 139      0   ${\registers}'_A = ({\registers}_B + \nu_X) \bmod 2^{64}$
  (lr)1-4 140      0   ${\registers}'_A = ({\registers}_B \cdot \nu_X) \bmod 2^{64}$
  (lr)1-4 141      0   ${\registers}'_A = \mathcal{X}_8(({\registers}_B \cdot 2^{\nu_X \bmod 64}) \bmod 2^{64})$
  (lr)1-4 142      0   ${\registers}'_A = \mathcal{X}_8(\floor{{\registers}_B \div 2^{\nu_X \bmod 64}})$
  (lr)1-4 143      0   ${\registers}'_A = \mathcal{Z}_{8}^{-1} (\floor{\mathcal{Z}_{8}({\registers}_B) \div 2^{\nu_X \bmod 64}})$
  (lr)1-4 144      0   ${\registers}'_A = (\nu_X + 2^{64} - {\registers}_B) \bmod 2^{64}$
  (lr)1-4 145      0   ${\registers}'_A = (\nu_X \cdot 2^{{\registers}_B \bmod 64}) \bmod 2^{64}$
  (lr)1-4 146      0   ${\registers}'_A = \floor{\nu_X \div 2^{{\registers}_B \bmod 64}}$
  (lr)1-4 147      0   ${\registers}'_A = \mathcal{Z}_{8}^{-1} (\floor{\mathcal{Z}_{8}(\nu_X) \div 2^{{\registers}_B \bmod 64}})$
  (lr)1-4              
  ------------- -- --- ----------------------------------------------------------------------------------------------------------------------------------
...
+:------------+:--+:--+:---------------------------------------------------------------------------------------------------------------------------------------------+
| 170         |   | 0 | ${\registers}'_D = \mathcal{X}_4(({\registers}_A + {\registers}_B) \bmod 2^{32})$                                                            |
+-------------+---+---+----------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 171 |   | 0 | ${\registers}'_D = \mathcal{X}_4(({\registers}_A + 2^{32} - ({\registers}_B \bmod 2^{32})) \bmod 2^{32})$                                    |
+-------------+---+---+----------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 172 |   | 0 | ${\registers}'_D = \mathcal{X}_4(({\registers}_A \cdot {\registers}_B) \bmod 2^{32})$                                                        |
+-------------+---+---+----------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 173 |   | 0 | ${\registers}'_D = \begin{cases}                                                                                                             |
|             |   |   |     2^{64} - 1 &\when {\registers}_B \bmod 2^{32} = 0\\                                                                                      |
|             |   |   |     \floor{({\registers}_A \bmod 2^{32}) \div ({\registers}_B \bmod 2^{32})} &\otherwise                                                     |
|             |   |   |   \end{cases}$                                                                                                                               |
+-------------+---+---+----------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 174 |   | 0 | ${\registers}'_D = \begin{cases}                                                                                                             |
|             |   |   |     2^{64} - 1 &\when b = 0\\                                                                                                                |
|             |   |   |     a &\when a = -2^{31} \wedge b = -1\\                                                                                                     |
|             |   |   |     \mathcal{Z}_{8}^{-1} (\floor{a \div b}) &\otherwise \\[2pt]                                                                              |
|             |   |   |     \multicolumn{2}{l}{\quad \where a = \mathcal{Z}_{4}({\registers}_A \bmod 2^{32})\,,\ b = \mathcal{Z}_{4}({\registers}_B \bmod 2^{32})}\\ |
|             |   |   |   \end{cases}$                                                                                                                               |
+-------------+---+---+----------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 175 |   | 0 | ${\registers}'_D = \begin{cases}                                                                                                             |
|             |   |   |     \mathcal{X}_4({\registers}_A) &\when {\registers}_B \bmod 2^{32} = 0\\                                                                   |
|             |   |   |     \mathcal{X}_4(({\registers}_A \bmod 2^{32}) \bmod ({\registers}_B \bmod 2^{32})) &\otherwise                                             |
|             |   |   |   \end{cases}$                                                                                                                               |
+-------------+---+---+----------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 176 |   | 0 | ${\registers}'_D = \begin{cases}                                                                                                             |
|             |   |   |     \mathcal{Z}_{8}^{-1} (a) &\when b = 0 \\                                                                                                 |
|             |   |   |     0 &\when a = -2^{31} \wedge b = -1 \\                                                                                                    |
|             |   |   |     \mathcal{Z}_{8}^{-1} (a \bmod b) &\otherwise \\[2pt]                                                                                     |
|             |   |   |     \multicolumn{2}{l}{\quad \where a = \mathcal{Z}_{4}({\registers}_A \bmod 2^{32})\,,\ b = \mathcal{Z}_{4}({\registers}_B \bmod 2^{32})}\\ |
|             |   |   |   \end{cases}$                                                                                                                               |
+-------------+---+---+----------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 177 |   | 0 | ${\registers}'_D = \mathcal{X}_4(({\registers}_A \cdot 2^{{\registers}_B \bmod 32}) \bmod 2^{32})$                                           |
+-------------+---+---+----------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 178 |   | 0 | ${\registers}'_D = \mathcal{X}_4(\floor{({\registers}_A \bmod 2^{32}) \div 2^{{\registers}_B \bmod 32}})$                                    |
+-------------+---+---+----------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 179 |   | 0 | ${\registers}'_D = \mathcal{Z}_{8}^{-1} (\floor{\mathcal{Z}_{4}({\registers}_A \bmod 2^{32}) \div 2^{{\registers}_B \bmod 32}})$             |
+-------------+---+---+----------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4     |   | 0 | ${\registers}'_D = ({\registers}_A + {\registers}_B) \bmod 2^{64}$                                                                           |
|             |   |   |                                                                                                                                              |
| 180         |   |   |                                                                                                                                              |
+-------------+---+---+----------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 181 |   | 0 | ${\registers}'_D = ({\registers}_A + 2^{64} - {\registers}_B) \bmod 2^{64}$                                                                  |
+-------------+---+---+----------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 182 |   | 0 | ${\registers}'_D = ({\registers}_A \cdot {\registers}_B) \bmod 2^{64}$                                                                       |
+-------------+---+---+----------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 183 |   | 0 | ${\registers}'_D = \begin{cases}                                                                                                             |
|             |   |   |     2^{64} - 1 &\when {\registers}_B = 0\\                                                                                                   |
|             |   |   |     \floor{{\registers}_A \div {\registers}_B} &\otherwise                                                                                   |
|             |   |   |   \end{cases}$                                                                                                                               |
+-------------+---+---+----------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 184 |   | 0 | ${\registers}'_D = \begin{cases}                                                                                                             |
|             |   |   |     2^{64} - 1 &\when {\registers}_B = 0\\                                                                                                   |
|             |   |   |     {\registers}_A &\when \mathcal{Z}_{8}({\registers}_A) = -2^{63} \wedge \mathcal{Z}_{8}({\registers}_B) = -1\\                            |
|             |   |   |     \mathcal{Z}_{8}^{-1} (\floor{\mathcal{Z}_{8}({\registers}_A) \div \mathcal{Z}_{8}({\registers}_B)}) &\otherwise                          |
|             |   |   |   \end{cases}$                                                                                                                               |
+-------------+---+---+----------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 185 |   | 0 | ${\registers}'_D = \begin{cases}                                                                                                             |
|             |   |   |     {\registers}_A &\when {\registers}_B = 0\\                                                                                               |
|             |   |   |     {\registers}_A \bmod {\registers}_B &\otherwise                                                                                          |
|             |   |   |   \end{cases}$                                                                                                                               |
+-------------+---+---+----------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 186 |   | 0 | ${\registers}'_D = \begin{cases}                                                                                                             |
|             |   |   |     {\registers}_A &\when {\registers}_B = 0\\                                                                                               |
|             |   |   |     0 &\when \mathcal{Z}_{8}({\registers}_A) = -2^{63} \wedge \mathcal{Z}_{8}({\registers}_B) = -1\\                                         |
|             |   |   |     \mathcal{Z}_{8}^{-1} (\mathcal{Z}_{8}({\registers}_A) \bmod \mathcal{Z}_{8}({\registers}_B)) &\otherwise                                 |
|             |   |   |   \end{cases}$                                                                                                                               |
+-------------+---+---+----------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 187 |   | 0 | ${\registers}'_D = ({\registers}_A \cdot 2^{{\registers}_B \bmod 64}) \bmod 2^{64}$                                                          |
+-------------+---+---+----------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 188 |   | 0 | ${\registers}'_D = \floor{{\registers}_A \div 2^{{\registers}_B \bmod 64}}$                                                                  |
+-------------+---+---+----------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 189 |   | 0 | ${\registers}'_D = \mathcal{Z}_{8}^{-1} (\floor{\mathcal{Z}_{8}({\registers}_A) \div 2^{{\registers}_B \bmod 64}})$                          |
+-------------+---+---+----------------------------------------------------------------------------------------------------------------------------------------------+
...
| 190         |   |   |                                                                                                                                              |
+-------------+---+---+----------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 191 |   | 0 | $\forall i \in \N_{64} : \mathcal{B}_{8}({\registers}'_D)_i = \mathcal{B}_{8}({\registers}_A)_i \oplus \mathcal{B}_{8}({\registers}_B)_i$    |
+-------------+---+---+----------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 192 |   | 0 | $\forall i \in \N_{64} : \mathcal{B}_{8}({\registers}'_D)_i = \mathcal{B}_{8}({\registers}_A)_i \vee \mathcal{B}_{8}({\registers}_B)_i$      |
+-------------+---+---+----------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 193 |   | 0 | ${\registers}'_D = \mathcal{Z}_{8}^{-1} (\floor{(\mathcal{Z}_{8}({\registers}_A) \cdot \mathcal{Z}_{8}({\registers}_B)) \div 2^{64}})$       |
+-------------+---+---+----------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 194 |   | 0 | ${\registers}'_D = \floor{({\registers}_A \cdot {\registers}_B) \div 2^{64}}$                                                                |
+-------------+---+---+----------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 195 |   | 0 | ${\registers}'_D = \mathcal{Z}_{8}^{-1} (\floor{(\mathcal{Z}_{8}({\registers}_A) \cdot {\registers}_B) \div 2^{64}})$                        |
+-------------+---+---+----------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 196 |   | 0 | ${\registers}'_D = {\registers}_A < {\registers}_B$                                                                                          |
+-------------+---+---+----------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 197 |   | 0 | ${\registers}'_D = \mathcal{Z}_{8}({\registers}_A) < \mathcal{Z}_{8}({\registers}_B)$                                                        |
+-------------+---+---+----------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 198 |   | 0 | ${\registers}'_D = \begin{cases}                                                                                                             |
|             |   |   |     {\registers}_A &\when {\registers}_B = 0\\                                                                                               |
|             |   |   |     {\registers}_D &\otherwise                                                                                                               |
|             |   |   |   \end{cases}$                                                                                                                               |
+-------------+---+---+----------------------------------------------------------------------------------------------------------------------------------------------+
| (lr)1-4 199 |   | 0 | ${\registers}'_D = \begin{cases}                                                                                                             |
...
\label{eq:conditions}
  ...
  &5\mathsf{Z}_Z + Q(|\mathbf{o}|) + Q(|\mathbf{w}| + z\mathsf{Z}_P) + Q(s) + \mathsf{Z}_I \leq 2^{32}\end{aligned}$$ Thus, if the above conditions cannot be satisfied with unique values, then the result is $\none$, otherwise it is a tuple of $\mathbf{c}$ as above and ${\memory}$, $\registers$ such that: $$\label{eq:memlayout}
...
The four instances where the [pvm]{.smallcaps} is utilized each expect to be able to pass argument data in and receive some return data back. We thus define the common [pvm]{.smallcaps} program-argument invocation function $\Psi_M$: $$\begin{aligned}
  ...
  \where R&\colon (\varepsilon, \imath', \gascounter', \registers', {\memory}', \mathbf{x}) \mapsto \begin{cases}
    ...
    (\gascounter', \memory'_{\registers'_{10}\dots+\registers'_{11}}, \mathbf{x}) &\when \varepsilon = \halt \wedge \mathbb{Z}_{\registers'_{10}\dots+\registers'_{11}} \subset \mathbb{V}_{{\memory}'} \\
...
Unlike the other invocation functions, the Refine invocation function implicitly draws upon some recent service account state item $\delta$. The specific block from which this comes is not important, as long as it is no earlier than its work-package's lookup-anchor block. It explicitly accepts the work payload, $\mathbf{y}$, together with the service index which is the subject of refinement $s$, the prediction of the hash of that service's code $c$ at the time of reporting, the hash of the containing work-package $p$, the refinement context $\mathbf{c}$, the authorizer hash $a$ and its output $\mathbf{o}$, and an export segment offset $\varsigma$, the import segments and extrinsic data blobs as dictated by the work-item, $\mathbf{i}$ and $\overline{\mathbf{x}}$. It results in either some error $\mathbb{J}$ or a pair of the refinement output blob and the export sequence. Formally: $$\begin{aligned}
  ...
  &F \in \Omega\ang{(\dict{\N}{\mathbf{M}}, \seq{\Y})} \colon
  ...
  **Gas usage**             
  ...
  $g = 10$                  $\begin{aligned}
                                ...
                                (\varepsilon', \registers'_7, (\mathbf{x}'_\mathbf{u})_\mathbf{d}, \mathbf{x}'_\mathbf{t}) &\equiv \begin{cases}
                                  ...
                                  (\blacktriangleright, \mathtt{OOB}, (\mathbf{x}_\mathbf{u})_\mathbf{d}, \mathbf{x}_\mathbf{t}) &\otherwhen t = \error \\
                                  (\blacktriangleright, \mathtt{WHO}, (\mathbf{x}_\mathbf{u})_\mathbf{d}, \mathbf{x}_\mathbf{t}) &\otherwhen d \not \in \keys{\mathbf{d}} \\
                                  (\blacktriangleright, \mathtt{LOW}, (\mathbf{x}_\mathbf{u})_\mathbf{d}, \mathbf{x}_\mathbf{t}) &\otherwhen g < \mathbf{d}[d]_m \\
  ...
  **Gas usage**              
  ...
  $g = 10$                   $\begin{aligned}
                                 ...
                                 (\registers'_7, \mathbf{e}') &\equiv \begin{cases}
                                   ...
                                   (\mathtt{FULL}, \mathbf{e}) &\otherwhen \varsigma+|\mathbf{e}| \ge \mathsf{W}_M \\
  ...
  $g = 10$                   $\begin{aligned}
                                 ...
                                 \using \mathbf{s} &= \begin{cases}
                                   ...
                                   \error &\otherwise
                                 \end{cases}\\
                                 (\registers'_7, {\memory}') &\equiv \begin{cases}
                                   (\mathtt{OOB}, {\memory}) &\when \mathbf{s} = \error \\
                                   (\mathtt{WHO}, {\memory}) &\when \mathbf{s} = \none \\
                                   (\mathtt{OK}, {\memory}') \ \where {\memory}' = {\memory}\exc {{\memory}'}^{\circlearrowleft}_{o\dots+z} = \mathbf{s} &\otherwise
                                 \end{cases} \\
                               \end{aligned}$
  (lr)1-1(lr)2-2             
  `poke` = 20                
  $g = 10$                   $\begin{aligned}
                                 ...
                                 \using \mathbf{s} &= \begin{cases}
                                   \none &\when n \not\in \keys{\mathbf{m}} \\
                                   {\memory}^{\circlearrowleft}_{s\dots+z} &\when \N_{s\dots+z} \subseteq \mathbb{V}_\memory \wedge \N_{o\dots+z} \subseteq \mathbb{V}^*_{\mathbf{m}[n]_\mathbf{u}} \\
                                   \error &\otherwise
                                 \end{cases}\\
                                 (\registers'_7, \mathbf{m}') &\equiv \begin{cases}
  ...
  $g = 10$                   $\begin{aligned}
                                 ...
                                 \using \mathbf{m}^* &= \mathbf{m} \exc \begin{cases}
                                   ...
                                   \end{cases}
                                 \end{cases}\\
                                 (\registers'_7, \registers'_8, {\memory}', \mathbf{m}') &\equiv \begin{cases}
                                   (\mathtt{OOB}, \registers_8, {\memory}, \mathbf{m}) &\when g = \error \\
                                   (\mathtt{WHO}, \registers_8, {\memory}, \mathbf{m}) &\otherwhen n \not\in \mathbf{m} \\
                                   (\mathtt{HOST}, h, {\memory}^*, \mathbf{m}^*) &\otherwhen c = \host \times h \\
                                   (\mathtt{FAULT}, x, {\memory}^*, \mathbf{m}^*) &\otherwhen c = \fault \times x \\
                                   (\mathtt{OOG}, \registers_8, {\memory}^*, \mathbf{m}^*) &\otherwhen c = \oog \\
                                   (\mathtt{PANIC}, \registers_8, {\memory}^*, \mathbf{m}^*) &\otherwhen c = \panic \\
                                   (\mathtt{HALT}, \registers_8, {\memory}^*, \mathbf{m}^*) &\otherwhen c = \halt \\
                                 \end{cases} \\
                               \end{aligned}$
  (lr)1-1(lr)2-2             
  `expunge` = 24             
