...
version: "0.4.1"
date: "Wed, 9 Oct 2024 18:55:04 +0900"
hash: "c71229bbed0fd9156ea7f38d6cb3398df0e80f38"
---
...
The [pvm]{.smallcaps} is a very simple [risc]{.smallcaps} *register machine* and as such has 13 registers, each of which is a 32-bit quantity, denoted as $\N_R$, a natural less than $2^{32}$.[^9] Within the context of the [pvm]{.smallcaps}, $\registers \in \seq{\N_R}_{13}$ is typically used to denote the registers. $$\begin{aligned}
  \mathbb{M} &\equiv \ltuple\isa{\mathbf{V}}{\Y_{2^{32}}}\ts\isa{\mathbf{A}}{\lseq\{\text{W}, \text{R}, \none\}\rseq_{2^{32}}}\rtuple\end{aligned}$$

The [pvm]{.smallcaps} assumes a simple pageable [ram]{.smallcaps} of 32-bit addressable octets where each octet may be either immutable, mutable or inaccessible. The [ram]{.smallcaps} definition $\mathbb{M}$ includes two components: a value $\mathbf{V}$ and access $\mathbf{A}$. If the component is unspecified while being subscripted then the value component may be assumed. Within the context of the virtual machine, $\memory \in \mathbb{M}$ is typically used to denote [ram]{.smallcaps}. $$\begin{aligned}
  \mathbb{V}_{\memory} \equiv \{i \mid \memory_\mathbf{A}[i] \ne \none \} \qquad
  \mathbb{V}^*_{\memory} \equiv \{i \mid \memory_\mathbf{A}[i] = \text{W} \}\end{aligned}$$
...
We limit the number of items in the segment-root lookup dictionary to 8, and the total serialized size of a work-report may be no greater than $\mathsf{W}_R$ bytes: $$\forall w \in \mathbb{W} : |w_\wrNsrlookup| \le 8\ \wedge\ |\se(w)| \leq \mathsf{W}_R$$
...
We define the queue-editing function $E$, which is essentially a mutator function for items such as those of $\ready$, parameterized by a dictionary such as the items in $\accumulated$. It is used to update queues of work-reports when some of them are accumulated. Functionally, it removes all entries whose work-report's hash is in the parameter's keys, removes any dependencies which appear in said set and, crucially, removes any entries whose segment-root correspondences diverge from those in the dictionary. Formally: $$E\colon\left\{\begin{aligned}
    ...
    &(\mathbf{r}, \mathbf{x}) \mapsto \left[
      ...
      \left\{\,\begin{aligned}
        ...
        &(w_s)_h \not\in \keys{\mathbf{x}},\\
        &\mathbf{x} \cup w_\wrNsrlookup = w_\wrNsrlookup \cup \mathbf{x}
      \end{aligned}\right.
      \right]
  \end{aligned}\right.$$

We further define the accumulation priority queue function $Q$, which provides the sequence of work-reports which are accumulatable given the work-reports which have hitherto been accumulated and a set of not-yet-accumulated work-reports and their dependencies. $$Q\colon\left\{\begin{aligned}
    &(\seq{(\mathbb{W}, \{\H\})}, \dict{\H}{\H}) \to \seq{\mathbb{W}} \\
    &(\mathbf{r}, \mathbf{a}) \mapsto \begin{cases}
      [] &\when \mathbf{g} = \{\} \\
      \mathbf{g} \frown Q(E(\mathbf{r}, P(\mathbf{g})), \mathbf{a} \cup P(\mathbf{g}))\!\!\!\! &\otherwise \\
      \multicolumn{2}{l}{\,\where \mathbf{g} = [w \mid (w, \{\}) \orderedin \mathbf{r}]}
    \end{cases}
  \end{aligned}\right.$$

Finally, we define the mapping function $P$ which builds a dictionary of work-package hashes to segment-roots from a set of work-reports: $$P\colon\left\{\begin{aligned}
    \{\mathbb{W}\} &\to \dict{\H}{\H}\\
...
We may now define the sequence of accumulatable work-reports in this block as $\mathbf{W}^*$: $$\begin{aligned}
  ...
  \mathbf{W}^* &\equiv \mathbf{W}^! \frown Q(\wideparen{\ready_{m\dots}} \concat \wideparen{\ready_{\dots m}} \concat \mathbf{W}^Q, \overbrace{\accumulated})\end{aligned}$$
...
Our formalisms begin by defining $\partialstate$ as a characterization (i.e. values capable of representing) of state components which are both needed and mutable by the accumulation process. This comprises the service accounts state (as in $\delta$), the upcoming validator keys $\iota$, the queue of work-reports $\varphi$ and the privileges state $\chi$. Formally: $$\label{eq:partialstate}
...
We denote the set characterizing a *deferred transfer* as $\defxfer$, noting that a transfer includes a memo component $m$ of $\mathsf{W}_T = 128$ octets, together with the service index of the sender $s$, the service index of the receiver $d$, the amount of tokens to be transferred $a$ and the gas limit $g$ for the transfer. Formally: $$\begin{aligned}
...
## 12.4 Deferred Transfers and State Integration

Given the result of the top-level $\Delta_+$, we may define the posterior state $\chi'$, $\varphi'$ and $\iota'$ as well as the second intermediate state of the service-accounts $\delta^\ddagger$ and the [Beefy]{.smallcaps} commitment map $\beefycommitmap$: $$\begin{aligned}
  \using (n, \mathbf{o}, \mathbf{t}, \beefycommitmap) &= \Delta_+(\mathsf{G}_T, \mathbf{W}^*, (\chi, \delta^\dagger, \iota, \varphi), \chi_\mathbf{g}) \\
  (\chi', \delta^\ddagger, \iota', \varphi') &\equiv \mathbf{o}\end{aligned}$$
...
The posterior state $\delta'$ may then be defined as the second intermediate state with all the deferred effects of the transfers applied: $$\delta' = \{ s \mapsto \Psi_T(\delta^\ddagger, a, R(\mathbf{t}, s)) \mid (s \mapsto a) \in \delta^\ddagger \}$$
...
The ability to communicate large amounts of data from one work-package to some subsequent work-package is a key feature of the JAM availability system. An export segment, defined as the set $\G$, is an octet sequence of fixed length $\mathsf{W}_S\mathsf{W}_E = 4104$. It is the smallest datum which may individually be imported from---or exported to---the long-term *Imports DA* during the Refine function of a work-package. Being an exact multiple of the erasure-coding piece size ensures that the data segments of work-package can be efficiently placed in the DA system. $$\label{eq:segment}
...
We limit the encoded size of a work-package plus the total size of the implied import and extrinsic items to 12[mb]{.smallcaps} in order to allow for around 2[mb]{.smallcaps}/s/core data throughput: $$\begin{aligned}
  ...
  \sum_{w\in p_\wpNworkitems} |w_\wiNimportsegments|\cdot\mathsf{W}_S\mathsf{W}_E + \sum_{w\in p_\wpNworkitems} \sum_{(h, l) \in w_\wiNextrinsics} \!\!\!l\right) \le \mathsf{W}_P \\
...
Formally: $$\label{eq:workresultfunction}
  \!\!\Xi \colon \left\{\begin{aligned}
    ...
    (p, c) &\mapsto \begin{cases}
        \error &\when \mathbf{o} \not\in \Y \\
        \tup{\is{a}{p_\wpNauthorizer}, \mathbf{o}, \is{x}{p_\wpNcontext}, s, \mathbf{l}, \mathbf{r}} \!\!\!\!\!&\otherwise
    \end{cases}
  \end{aligned}\right.\!\!\!\!\!\!\!$$
...
\end{aligned}$$
  ...
  ------------ -- --- ----------------------------------------------------------------------------
  62              0   ${{\memory}'}^\circlearrowleft_{\nu_X} = \nu_Y \bmod 2^8$
  (lr)1-4 79      0   ${{\memory}'}^\circlearrowleft_{\nu_X\dots+2} = \se_2(\nu_Y \bmod 2^{16})$
  (lr)1-4 38      0   ${{\memory}'}^\circlearrowleft_{\nu_X\dots+4} = \se_4(\nu_Y)$
  ------------ -- --- ----------------------------------------------------------------------------
...
\end{aligned}$$
  ...
  ------------ -- --- ----------------------------------------------------------------------------------------------------------------
  19              0   $\token{djump}(({\registers}_A + \nu_X) \bmod 2^{32})$
  (lr)1-4 4       0   ${\registers}'_A = \nu_X$
  (lr)1-4 60      0   ${\registers}'_A = {\memory}_{\nu_X}$
  (lr)1-4 74      0   ${\registers}'_A = \mathcal{Z}_{4}^{-1} (\mathcal{Z}_{1}({\memory}_{\nu_X}))$
  (lr)1-4 76      0   ${\registers}'_A = \de_2({\memory}^{\circlearrowleft}_{\nu_X\dots+2})$
  (lr)1-4 66      0   ${\registers}'_A = \mathcal{Z}_{4}^{-1} (\mathcal{Z}_{2}(\de_2({\memory}^{\circlearrowleft}_{\nu_X\dots+2})))$
  (lr)1-4 10      0   ${\registers}'_A = \de_4({\memory}^{\circlearrowleft}_{\nu_X\dots+4})$
  (lr)1-4 71      0   ${{\memory}'}^{\circlearrowleft}_{\nu_X} = {\registers}_A \bmod 2^8$
  (lr)1-4 69      0   ${{\memory}'}^{\circlearrowleft}_{\nu_X\dots+2} = \se_2({\registers}_A \bmod 2^{16})$
  (lr)1-4 22      0   ${{\memory}'}^{\circlearrowleft}_{\nu_X\dots+4} = \se_4({\registers}_A)$
  ------------ -- --- ----------------------------------------------------------------------------------------------------------------
...
\end{aligned}$$
  ...
  ------------ -- --- -------------------------------------------------------------------------------------------------
  26              0   ${{\memory}'}^{\circlearrowleft}_{{\registers}_A + \nu_X} = \nu_Y \bmod 2^8$
  (lr)1-4 54      0   ${{\memory}'}^{\circlearrowleft}_{{\registers}_A + \nu_X \dots+ 2} = \se_2(\nu_Y \bmod 2^{16})$
  (lr)1-4 13      0   ${{\memory}'}^{\circlearrowleft}_{{\registers}_A + \nu_X \dots+ 4} = \se_4(\nu_Y)$
  ------------ -- --- -------------------------------------------------------------------------------------------------
...
$$\begin{aligned}
  ...
  (lr)1-4 32      0   $\token{branch}(\nu_Y, \mathcal{Z}_{4}({\registers}_A) < \mathcal{Z}_{4}(\nu_X))$
  (lr)1-4 46      0   $\token{branch}(\nu_Y, \mathcal{Z}_{4}({\registers}_A) \le \mathcal{Z}_{4}(\nu_X))$
  (lr)1-4 45      0   $\token{branch}(\nu_Y, \mathcal{Z}_{4}({\registers}_A) \ge \mathcal{Z}_{4}(\nu_X))$
...
\end{aligned}$$
  ...
  ------------ -- --- ----------------------------------------------------------------------------------------------------------------------------------
  16              0   ${{\memory}'}^{\circlearrowleft}_{{\registers}_B + \nu_X} = {\registers}_A \bmod 2^8$
  (lr)1-4 29      0   ${{\memory}'}^{\circlearrowleft}_{{\registers}_B + \nu_X \dots+ 2} = \se_2({\registers}_A \bmod 2^{16})$
  (lr)1-4 3       0   ${{\memory}'}^{\circlearrowleft}_{{\registers}_B + \nu_X \dots+ 4} = \se_4({\registers}_A)$
  (lr)1-4 11      0   ${\registers}'_A = {\memory}_{{\registers}_B + \nu_X}$
  (lr)1-4 21      0   ${\registers}'_A = \mathcal{Z}_{4}^{-1} (\mathcal{Z}_{1}({\memory}_{{\registers}_B + \nu_X}))$
  (lr)1-4 37      0   ${\registers}'_A = \de_2({\memory}^{\circlearrowleft}_{{\registers}_B + \nu_X\dots+2})$
  (lr)1-4 33      0   ${\registers}'_A = \mathcal{Z}_{4}^{-1} (\mathcal{Z}_{2}(\de_2({\memory}^{\circlearrowleft}_{{\registers}_B + \nu_X\dots+2})))$
  (lr)1-4 1       0   ${\registers}'_A = \de_4({\memory}^{\circlearrowleft}_{{\registers}_B + \nu_X\dots+4})$
  (lr)1-4 2       0   ${\registers}'_A = ({\registers}_B + \nu_X) \bmod 2^{32}$
  (lr)1-4 18      0   $\forall i \in \N_{32} : \mathcal{B}_{4}({\registers}'_A)_i = \mathcal{B}_{4}({\registers}_B)_i \wedge \mathcal{B}_{4}(\nu_X)_i$
  (lr)1-4 31      0   $\forall i \in \N_{32} : \mathcal{B}_{4}({\registers}'_A)_i = \mathcal{B}_{4}({\registers}_B)_i \oplus \mathcal{B}_{4}(\nu_X)_i$
  (lr)1-4 49      0   $\forall i \in \N_{32} : \mathcal{B}_{4}({\registers}'_A)_i = \mathcal{B}_{4}({\registers}_B)_i \vee \mathcal{B}_{4}(\nu_X)_i$
  (lr)1-4 35      0   ${\registers}'_A = ({\registers}_B \cdot \nu_X) \bmod 2^{32}$
  (lr)1-4 65      0   ${\registers}'_A = \mathcal{Z}_{4}^{-1} (\floor{(\mathcal{Z}_{4}({\registers}_B) \cdot \mathcal{Z}_{4}(\nu_X)) \div 2^{32}})$
  (lr)1-4 63      0   ${\registers}'_A = \floor{({\registers}_B \cdot \nu_X) \div 2^{32}}$
  (lr)1-4 27      0   ${\registers}'_A = {\registers}_B < \nu_X$
  (lr)1-4 56      0   ${\registers}'_A = \mathcal{Z}_{4}({\registers}_B) < \mathcal{Z}_{4}(\nu_X)$
  (lr)1-4 9       0   ${\registers}'_A = ({\registers}_B \cdot 2^{\nu_X \bmod 32}) \bmod 2^{32}$
  (lr)1-4 14      0   ${\registers}'_A = \floor{{\registers}_B \div 2^{\nu_X \bmod 32}}$
  (lr)1-4 25      0   ${\registers}'_A = \mathcal{Z}_{4}^{-1} (\floor{\mathcal{Z}_{4}({\registers}_B) \div 2^{\nu_X \bmod 32}})$
  (lr)1-4 40      0   ${\registers}'_A = (\nu_X + 2^{32} - {\registers}_B) \bmod 2^{32}$
  (lr)1-4 39      0   ${\registers}'_A = {\registers}_B > \nu_X$
  (lr)1-4 61      0   ${\registers}'_A = \mathcal{Z}_{4}({\registers}_B) > \mathcal{Z}_{4}(\nu_X)$
  (lr)1-4 75      0   ${\registers}'_A = (\nu_X \cdot 2^{{\registers}_B \bmod 32}) \bmod 2^{32}$
  (lr)1-4 72      0   ${\registers}'_A = \floor{\nu_X \div 2^{{\registers}_B \bmod 32}}$
  (lr)1-4 80      0   ${\registers}'_A = \mathcal{Z}_{4}^{-1} (\floor{\mathcal{Z}_{4}(\nu_X) \div 2^{{\registers}_B \bmod 32}})$
  (lr)1-4 85      0   ${\registers}'_A = \begin{cases}
                          \nu_X &\when {\registers}_B = 0\\
                          {\registers}_A &\otherwise
                        \end{cases}$
  (lr)1-4 86      0   ${\registers}'_A = \begin{cases}
...
$$\begin{aligned}
  ...
  (lr)1-4 48      0   $\token{branch}(\nu_X, \mathcal{Z}_{4}({\registers}_A) < \mathcal{Z}_{4}({\registers}_B))$
  (lr)1-4 41      0   $\token{branch}(\nu_X, {\registers}_A \ge {\registers}_B)$
  (lr)1-4 43      0   $\token{branch}(\nu_X, \mathcal{Z}_{4}({\registers}_A) \ge \mathcal{Z}_{4}({\registers}_B))$
  ------------ -- --- ----------------------------------------------------------------------------------------------
...
\end{aligned}$$
  ...
  ------------ -- --- -------------------------------------------------------------------------------------------------------------------------------------------
  8               0   ${\registers}'_D = ({\registers}_A + {\registers}_B) \bmod 2^{32}$
  (lr)1-4 20      0   ${\registers}'_D = ({\registers}_A + 2^{32} - {\registers}_B) \bmod 2^{32}$
  (lr)1-4 23      0   $\forall i \in \N_{32} : \mathcal{B}_{4}({\registers}'_D)_i = \mathcal{B}_{4}({\registers}_A)_i \wedge \mathcal{B}_{4}({\registers}_B)_i$
  (lr)1-4 28      0   $\forall i \in \N_{32} : \mathcal{B}_{4}({\registers}'_D)_i = \mathcal{B}_{4}({\registers}_A)_i \oplus \mathcal{B}_{4}({\registers}_B)_i$
  (lr)1-4 12      0   $\forall i \in \N_{32} : \mathcal{B}_{4}({\registers}'_D)_i = \mathcal{B}_{4}({\registers}_A)_i \vee \mathcal{B}_{4}({\registers}_B)_i$
  (lr)1-4 34      0   ${\registers}'_D = ({\registers}_A \cdot {\registers}_B) \bmod 2^{32}$
  (lr)1-4 67      0   ${\registers}'_D = \mathcal{Z}_{4}^{-1} (\floor{(\mathcal{Z}_{4}({\registers}_A) \cdot \mathcal{Z}_{4}({\registers}_B)) \div 2^{32}})$
  (lr)1-4 57      0   ${\registers}'_D = \floor{({\registers}_A \cdot {\registers}_B) \div 2^{32}}$
  (lr)1-4 81      0   ${\registers}'_D = \mathcal{Z}_{4}^{-1} (\floor{(\mathcal{Z}_{4}({\registers}_A) \cdot {\registers}_B) \div 2^{32}})$
  (lr)1-4 68      0   ${\registers}'_D = \begin{cases}
                          2^{32} - 1 &\when {\registers}_B = 0\\
                          \floor{{\registers}_A \div {\registers}_B} &\otherwise
                        \end{cases}$
  (lr)1-4 64      0   ${\registers}'_D = \begin{cases}
                          2^{32} - 1 &\when {\registers}_B = 0\\
                          {\registers}_A &\when \mathcal{Z}_{4}({\registers}_A) = -2^{31} \wedge \mathcal{Z}_{4}({\registers}_B) = -1\\
                          \mathcal{Z}_{4}^{-1} (\floor{\mathcal{Z}_{4}({\registers}_A) \div \mathcal{Z}_{4}({\registers}_B)}) &\otherwise
                        \end{cases}$
  (lr)1-4 73      0   ${\registers}'_D = \begin{cases}
                          {\registers}_A &\when {\registers}_B = 0\\
                          {\registers}_A \bmod {\registers}_B &\otherwise
                        \end{cases}$
  (lr)1-4 70      0   ${\registers}'_D = \begin{cases}
                          {\registers}_A &\when {\registers}_B = 0\\
                          0 &\when \mathcal{Z}_{4}({\registers}_A) = -2^{31} \wedge \mathcal{Z}_{4}({\registers}_B) = -1\\
                          \mathcal{Z}_{4}^{-1} (\mathcal{Z}_{4}({\registers}_A) \bmod \mathcal{Z}_{4}({\registers}_B)) &\otherwise
                        \end{cases}$
  (lr)1-4 36      0   ${\registers}'_D = {\registers}_A < {\registers}_B$
  (lr)1-4 58      0   ${\registers}'_D = \mathcal{Z}_{4}({\registers}_A) < \mathcal{Z}_{4}({\registers}_B)$
  (lr)1-4 55      0   ${\registers}'_D = ({\registers}_A \cdot 2^{{\registers}_B \bmod 32}) \bmod 2^{32}$
  (lr)1-4 51      0   ${\registers}'_D = \floor{{\registers}_A \div 2^{{\registers}_B \bmod 32}}$
  (lr)1-4 77      0   ${\registers}'_D = \mathcal{Z}_{4}^{-1} (\floor{\mathcal{Z}_{4}({\registers}_A) \div 2^{{\registers}_B \bmod 32}})$
  (lr)1-4 83      0   ${\registers}'_D = \begin{cases}
                          {\registers}_A &\when {\registers}_B = 0\\
                          {\registers}_D &\otherwise
                        \end{cases}$
  (lr)1-4 84      0   ${\registers}'_D = \begin{cases}
...
An extended version of the [pvm]{.smallcaps} invocation which is able to progress an inner *host-call* state-machine in the case of a host-call halt condition is defined as $\Psi_H$: $$\begin{aligned}
  &\Psi_H\colon \left\{\begin{aligned}
    ...
    &\to
    \tuple{\{\panic, \oog, \halt\} \cup \{\fault\} \times \N_R, \Z_G, \seq{\N_R}_{13}, \mathbb{M}, X}\\
...
\label{eq:conditions}
  ...
  &5\mathsf{Z}_Q + Q(|\mathbf{o}|) + Q(|\mathbf{w}| + z\mathsf{Z}_P) + Q(s) + \mathsf{Z}_I \leq 2^{32}\end{aligned}$$ Thus, if the above conditions cannot be satisfied with unique values, then the result is $\none$, otherwise it is a tuple of $\mathbf{c}$ as above and ${\memory}$, $\registers$ such that: $$\label{eq:memlayout}
  \forall i \in \N_R : {\memory}_i = \left\{\begin{alignedat}{5}
...
$$\begin{aligned}
  ...
  \label{eq:isauthorizedmutator}F&\colon\left\{\begin{aligned}
    (\N, \N_G, \seq{\N_R}_{13}, \mathbb{M}) &\to (\Z_G, \seq{\N_R}_{13}, \mathbb{M})\\
...
Unlike the other invocation functions, the Refine invocation function implicitly draws upon some recent service account state item $\delta$. The specific block from which this comes is not important, as long as it is no earlier than its work-package's lookup-anchor block. It explicitly accepts the work payload, $\mathbf{y}$, together with the service index which is the subject of refinement $s$, the prediction of the hash of that service's code $c$ at the time of reporting, the hash of the containing work-package $p$, the refinement context $\mathbf{c}$, the authorizer hash $a$ and its output $\mathbf{o}$, and an export segment offset $\varsigma$, the import segments and extrinsic data blobs as dictated by the work-item, $\mathbf{i}$ and $\overline{\mathbf{x}}$. It results in either some error $\mathbb{J}$ or a pair of the refinement output blob and the export sequence. Formally: $$\begin{aligned}
  ...
  F&\colon\left\{\begin{aligned}
    (\N, \N_G, \seq{\N_R}_{13}, \mathbb{M}, (\dict{\N}{\mathbf{M}}, \seq{\Y})) &\to (\N_G, \seq{\N_R}_{13}, \mathbb{M}, (\dict{\N}{\mathbf{M}}, \seq{\Y}))\\
    (n, \gascounter, \registers, \memory, (\mathbf{m}, \mathbf{e})) &\mapsto \begin{cases}
      ...
      \Omega_Z(\gascounter, \registers, \memory, (\mathbf{m}, \mathbf{e}), \varsigma) &\when n = \mathtt{export}\\
...
We define $\Psi_A$, the Accumulation invocation function as: $$\Psi_A \colon\left\{\begin{aligned}
    \tuple{
...
These assume some refine context pair $(\mathbf{m}, \mathbf{e}) \in (\dict{\N}{\mathbf{M}}, \seq{\Y_{\mathsf{W}_S}})$, which are both initially empty. Other than the gas-counter which is explicitly defined, elements of [pvm]{.smallcaps} state are each assumed to remain unchanged by the host-call unless explicitly specified. $$\begin{aligned}
  \gascounter' &\equiv \gascounter - g\\
  ...
  **Gas usage**              
  ...
  $g = 10$                   $\begin{aligned}
                                 ...
                                 \using \mathbf{x} &= \begin{cases}
                                   \mathcal{P}_{\mathsf{W}_E\mathsf{W}_S}({\memory}_{p\dots+z}) &\when \N_{p\dots+z} \in \mathbb{V}_\memory\\
                                   \error &\otherwise
                                 \end{cases}\\
  ...
  $g = 10$                   $\begin{aligned}
                                 ...
                                 (\registers'_7, {\memory}') &\equiv \begin{cases}
                                   ...
                                   (\mathtt{OK}, {\memory}') \ \where {\memory}' = {\memory}\exc {\memory}'_{a\dots+l} = \mathbf{s} &\otherwise
                                 \end{cases} \\
                               \end{aligned}$
  (lr)1-1(lr)2-2             
  `poke` = 20                
  $g = 10$                   $\begin{aligned}
                                 ...
                                 \end{cases} \\
                                 \using \mathbf{s} &= \begin{cases}
                                   {\memory}_{a\dots+i} &\when \N_{a\dots+i} \in \mathbb{V}_\mathbf{u}\\
                                   \error &\otherwise
                                 \end{cases}\\
  ...
  $g = 10$                   $\begin{aligned}
                                 ...
                                 \using (g, \mathbf{w}) &= \begin{cases}
                                   (\de_8({\memory}_{o\dots+8}), [\de_4({\memory}_{o+8+4x\dots+4}) \mid x \orderedin \N_{13}]) &\when \N_{o \dots+ 60} \subset \mathbb{V}^*_{{\memory}} \\
                                   (\error, \error) &\otherwise
                                 \end{cases} \\
                                 \using (c, i', g', \mathbf{w}', \mathbf{u}') &= \Psi(\mathbf{m}[n]_\mathbf{p}, \mathbf{m}[n]_i, g, \mathbf{w}, \mathbf{m}[n]_\mathbf{u})\\
                                 \using {\memory}^* &= {\memory}\exc {\memory}^*_{o\dots+60} = \se_8(g') \concat \joined{\se_4^\#(\mathbf{w}')}\\
...
:   The total gas allocated across all cores for Accumulation. May be no smaller than $\mathsf{G}_A\cdot\mathsf{C} + \sum_{g \in \mathcal{V}(\chi_\mathbf{g})}(g)$.
...
:   The size of an exported segment in erasure-coded pieces in octets.
