...
version: "0.4.5"
date: "Thu, 31 Oct 2024 13:59:06 +0100"
hash: "364735a56c5095287cd854597fadd32c20ece8a7"
---
...
The posterior state $\delta'$ may then be defined as the second intermediate state with all the deferred effects of the transfers applied: $$\delta' = \{ s \mapsto \Psi_T(\delta^\ddagger, s, R(\mathbf{t}, s)) \mid (s \mapsto a) \in \delta^\ddagger \}$$
...
These assume some refine context pair $(\mathbf{m}, \mathbf{e}) \in (\dict{\N}{\mathbf{M}}, \seq{\Y_{\mathsf{W}_S}})$, which are both initially empty. Other than the gas-counter which is explicitly defined, elements of [pvm]{.smallcaps} state are each assumed to remain unchanged by the host-call unless explicitly specified. $$\begin{aligned}
  ...
  **Gas usage**              
  ...
  $g = 10$                   $\begin{aligned}
                                 ...
                                 \end{cases} \\
                                 \using \mathbf{s} &= \begin{cases}
                                   {\memory}_{a\dots+l} &\when \N_{a\dots+l} \in \mathbb{V}_\mathbf{u}\\
                                   \error &\otherwise
                                 \end{cases}\\
